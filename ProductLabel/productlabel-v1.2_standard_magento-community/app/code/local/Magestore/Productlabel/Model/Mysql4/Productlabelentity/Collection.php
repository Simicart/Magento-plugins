 ...
 */
class Magestore_Productlabel_Model_Mysql4_Productlabelentity_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract {
        ...
        if ($storeId = $this->getStoreId()) {
            ...
            if (in_array($field, $attributes)) {
                if (!in_array($field, $this->_addedTable)) {
                    $this->getSelect()
                            ->joinLeft(array($field => $this->getTable('productlabel/productlabelentityvalue')), "main_table.account_id = $field.account_id" .
