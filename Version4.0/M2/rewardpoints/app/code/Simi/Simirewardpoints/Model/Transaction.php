...
class Transaction extends \Magento\Framework\Model\AbstractModel
{
     ...
     * @param \Simi\Simirewardpoints\Helper\Customer $rewardpointsHelperCustomer
     * @param \Simi\Simirewardpoints\Helper\Action $helperAction
     * @param array $data
     */
    public function __construct(
        ...
        \Simi\Simirewardpoints\Model\Customer $rewardpointsCustomer,
        ...
        \Simi\Simirewardpoints\Helper\Customer $rewardpointsHelperCustomer,
        \Simi\Simirewardpoints\Helper\Action $helperAction,
        ...
        $this->rewardAccount = $rewardpointsCustomer;
    ...
    {
        $this->_init('Simi\Simirewardpoints\Model\ResourceModel\Transaction');
    }
    ...
    public function completeTransaction()
    {
        ...
        if (!$this->getId() || !$this->getCustomerId() || !$this->getRewardId() || $this->getPointAmount() <= 0 || !in_array($this->getStatus(), [self::STATUS_PENDING, self::STATUS_ON_HOLD])
        ) {
    ...
    public function getRewardAccount()
    {
        if (!$this->hasData('reward_account')) {
            $this->setData('reward_account', $this->rewardAccount->load($this->getRewardId()));
        }
     ...
     *
     * @param \Simi\Simirewardpoints\Model\Customer $rewardAccount
    ...
    public function sendUpdateBalanceEmail($rewardAccount = null)
    {
        ...
        if (!$customer) {
            $customer = $this->_modelCustomer->load($rewardAccount->getCustomerId());
        }
        if (!$customer->getId()) {
            return $this;
        }
...
//      $translate->setTranslateInline(true);
    ...
    public function createTransaction($data = [])
    {
        ...
        $this->_eventManager->dispatch($this->_eventPrefix . '_created_' . $this->getData('action'), $this->_getEventData());
     ...
     *
     * @return \Simi\Simirewardpoints\Model\InterfaceAction
     */
