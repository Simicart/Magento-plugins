...
namespace Simi\Simirewardpoints\Model\ResourceModel;
...
class Transaction extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('simirewardpoints_transaction', 'transaction_id');
    }
        ...
        try {
            foreach ($customers as $customerReward) {
                ...
                $preTransaction = [
                    ...
                    'action' => 'admin',
                    'action_type' => \Simi\Simirewardpoints\Model\Transaction::ACTION_TYPE_EARN,
                ...
                $write->update($this->getTable('simirewardpoints_customer'), $preReward, "customer_id = $customer_id");
                if (count($dataTransaction) >= 1000) {
                    $write->insertMultiple($this->getTable('simirewardpoints_transaction'), $dataTransaction);
                    $dataTransaction = [];
                }
            }
     ...
     * @return \Simi\Simirewardpoints\Model\ResourceModel\Transaction
     */
    public function updatePointUsed($transaction)
    {
        ...
        $selectSql = $read->select()->reset()
                ...
                ->where('status = ?', \Simi\Simirewardpoints\Model\Transaction::STATUS_COMPLETED)
     ...
     *
     * @param \Simi\Simirewardpoints\Model\Transaction $transaction
     * @return \Simi\SimirewardpointsModel\ResourceModel\Transaction
     */
    public function updateRealPointHolding($transaction)
    {
        ...
        $selectSql = $read->select()->reset()
                ...
                ->where('action_type = ?', \Simi\Simirewardpoints\Model\Transaction::ACTION_TYPE_EARN)
                ->where('point_amount > point_used')
                ->where('status = ?', \Simi\Simirewardpoints\Model\Transaction::STATUS_ON_HOLD);
