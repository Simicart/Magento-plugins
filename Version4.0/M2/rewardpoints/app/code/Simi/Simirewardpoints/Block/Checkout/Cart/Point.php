...
/**
 * Simirewardpoints Show Spending Point on Shopping Cart Page
 *
 * @category    Simi
...
namespace Simi\Simirewardpoints\Block\Checkout\Cart;
...
class Point extends \Magento\Framework\View\Element\Template
{
     ...
     * @param \Simi\Simirewardpoints\Helper\Policy $helperPolicy
     */
    public function __construct(
        ...
        \Simi\Simirewardpoints\Helper\Policy $helperPolicy
    ) {
     ...
     *
     * @return \Simi\Simirewardpoints\Helper\Block\Spend
     */
    ...
    public function knockoutCheckRule()
    {
        ...
        if ($_rule->getId() == 'rate') {
            return __('Each of %1 gets %2 discount', $_pointHelper->format($_rule->getPointsSpended()), $this->formatDiscount($_rule));
        } else {
    ...
    {
        $customer = $this->_helperCustomer;
        $_spendingHelper = $this->_calculationSpending;
        $checkout = $this->_checkoutSession;

        if ($customer->getBalance() && $_spendingHelper->isUseMaxPointsDefault()
                && (!$_spendingHelper->isUsePoint() || $checkout->getData('use_max'))) {
            return 1;
        } else {
            return 0;
        }
    ...
    public function checkEarnwhenSpend()
    {
        if (!$this->_scopeConfig->getValue(
            'simirewardpoints/earning/earn_when_spend',
    ...
    {
        $max_default_point = $this->_scopeConfig->getValue(
            'simirewardpoints/spending/max_points_per_order',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    ...
    {
        return $this->_scopeConfig->getValue('simirewardpoints/spending/max_point_default', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
    }
    ...
    public function knockoutData()
    {
        ...
        if (count($this->getSliderRules()) || count($this->getCheckboxRules())) {
            ...
            $results['urlUpdateTotal'] = $this->createUrl('simirewardpoints/checkout/updateTotal/');
            $results['urlRefreshPoint'] = $this->createUrl('simirewardpoints/checkout/refreshPoint/');
        } else {
            $results['enableReward'] = false;
        }
        if ($this->getNameInLayout() == 'rewardpoint_checkout_cart_point') {
            $results['template'] = 'Simi_Simirewardpoints/checkout/summary/spending';
        } elseif ($this->getNameInLayout() == 'rewardpoint_checkout_index_point') {
            $results['template'] = 'Simi_Simirewardpoints/checkout/payment/spending';
        }
        return $results;
    }
    ...
    public function getJsLayout()
    {
        return \Zend_Json::encode([
                    'components' => [
                        'block-rewardpoints' => [
                            'component' => 'Simi_Simirewardpoints/js/view/checkout/payment/spending',
                            'sortOrder' => 20
                        ]
                    ]
        ]);
