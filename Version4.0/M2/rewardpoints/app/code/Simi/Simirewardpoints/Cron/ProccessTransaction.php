...
namespace Simi\Simirewardpoints\Cron;
...
class ProccessTransaction
{
    ...
    {
        \Magento\Framework\Profiler::start('SIMIREWARDPOINTS_CRON::processTransactions');
        $stores = [];
        $allStores = true;

        foreach ($this->_storeManager->getStores(true) as $_store) {
            if ($this->_helper->getConfig(\Simi\Simirewardpoints\Helper\Data::XML_PATH_ENABLE, $_store)) {
        ...
        // expire transactions
        \Magento\Framework\Profiler::start('SIMIREWARDPOINTS_CRON::expireTransactions');

        $expireTrans = $this->_transaction->create()->getCollection()
                ->addFieldToFilter('status', ['lteq' => \Simi\Simirewardpoints\Model\Transaction::STATUS_COMPLETED])
        ...
        unset($expireTrans);
        \Magento\Framework\Profiler::stop('SIMIREWARDPOINTS_CRON::expireTransactions');

        // send before expire email to customer
        $beforeDays = [];
        foreach ($stores as $_store) {
            if (!$this->_helper->getConfig(\Simi\Simirewardpoints\Model\Transaction::XML_PATH_EMAIL_ENABLE, $_store)) {
                $allStores = false;
                continue;
            }
            $_beforeDays = (int) $this->_helper->getConfig(
                \Simi\Simirewardpointss\Model\Transaction::XML_PATH_EMAIL_EXPIRE_DAYS,
                $_store
            );
            if ($_beforeDays <= 0) {
                $allStores = false;
        ...
        \Magento\Framework\Profiler::stop('SIMIREWARDPOINTS_CRON::processTransactions');
    }
    ...
    {
        \Magento\Framework\Profiler::start('SIMIREWARDPOINTS_CRON::sendBeforeExpireEmail');
        ...
        $transactions = $this->_transaction->create()->getCollection()
                ->addFieldToFilter('status', \Simi\Simirewardpoints\Model\Transaction::STATUS_COMPLETED)
        ...
        if (!count($transactions)) {
            \Magento\Framework\Profiler::stop('SIMIREWARDPOINTS_CRON::sendBeforeExpireEmail');
            return;
        }
        $rewardAccount = $this->_rewardCustomer;
        $customer = $this->_customer;
    ...
    {
        \Magento\Framework\Profiler::start('SIMIREWARDPOINTS_CRON::completeHoldingTransaction');
        $releaseTime = date('Y-m-d H:i:s', time() - $holdDays * 86400);
        $holdingTrans = $this->_transaction->create()->getCollection()
                ->addFieldToFilter('status', \Simi\Simirewardpoints\Model\Transaction::STATUS_ON_HOLD)
                ->addFieldToFilter('created_time', ['to' => $releaseTime]);
        if (is_array($storeIds) && count($storeIds)) {
            $holdingTrans->addFieldToFilter('store_id', ['in' => $storeIds]);
        }
        if (!count($holdingTrans)) {
            \Magento\Framework\Profiler::stop('SIMIREWARDPOINTS_CRON::completeHoldingTransaction');
            return;
        }
        $rewardAccount = $this->_rewardCustomer;
        $customer = $this->_customer;
