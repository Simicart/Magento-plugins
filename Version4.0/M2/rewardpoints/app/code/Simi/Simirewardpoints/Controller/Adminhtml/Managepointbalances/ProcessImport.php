...
class ProcessImport extends \Magento\Backend\App\Action
{
        ...
        if (isset($files['name']) && $files['name'] != '') {
            try {
                if (pathinfo($files['name'], PATHINFO_EXTENSION) != 'csv') {
                    ...
                    $resultRedirect->setPath('simirewardpoints/managepointbalances/import');
                ...
                if (isset($customerData) && count($customerData)) {
                    ...
                    /** @var \Magento\Backend\Model\View\Result\Redirect $resultRedirect */
                    $resultRedirect = $this->resultFactory->create(ResultFactory::TYPE_REDIRECT);
                    $resultRedirect->setPath('simirewardpoints/managepointbalances/index');
                    return $resultRedirect;
                } else {
    ...
    protected function _updateCustomer($customerData)
    {
        ...
        $this->_objectManager->create('Simi\Simirewardpoints\Model\ResourceModel\Transaction')->importPointFromCsv($collection);
