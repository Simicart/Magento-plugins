...
namespace Simi\Simipushnotif\Setup;
...
class InstallSchema implements InstallSchemaInterface
{
    public function install(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($context->getVersion(), '1.0.0') < 0) {
            $installer->run("DROP TABLE IF EXISTS {$installer->getTable('simipush_device')};");
            $installer->run("CREATE TABLE {$installer->getTable('simipush_device')} (
            ...
            $installer->run("DROP TABLE IF EXISTS {$installer->getTable('simipush_message')};");
            $installer->run("CREATE TABLE {$installer->getTable('simipush_message')} (
