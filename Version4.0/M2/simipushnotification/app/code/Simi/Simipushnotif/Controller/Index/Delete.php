...
class Delete extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $agent = $this->_objectManager->get('Simi\Simipushnotif\Model\Device')->load($dataAgent['endpoint'], 'endpoint');
        if ($agent->getId()) {
            try {
                $message = $this->_objectManager->get('Simi\Simipushnotif\Model\Notification')->load($agent->getId(), 'device_id');
                if ($message->getId()) {
                    $message->delete();
                }
                $agent->delete();
