...
class Simi_Simigiftvoucher_Model_Api extends Mage_Api_Model_Resource_Abstract
{
    ...
    public function check($code)
    {
        ...
        if (!$giftVoucher->getId()) {
            $this->_fault('not_exists', Mage::helper('simigiftvoucher')->__('Gift card "%s" is not exists', $code));
        }
        ...
        try {
            if (!isset($data['balance']) || $data['balance'] <= 0) {
                $this->_fault('data_invalid', 
                    Mage::helper('simigiftvoucher')->__('Your Gift Card credit balance must be greater than 0.'));
            }
    ...
    public function massCreate($data)
    {
        ...
        try {
            ...
            if (!Mage::helper('simigiftvoucher')->isExpression($data['pattern'])) {
                $this->_fault('data_invalid', 
                    Mage::helper('simigiftvoucher')->__('Invalid pattern'));
            }
            $data = $this->processData($data);
            ...
            $giftcard['status'] = Simi_Simigiftvoucher_Model_Status::STATUS_ACTIVE;
            $amount = $model->getAmount();
            $result = array();
            for ($i = 1; $i <= $amount; $i++) {
                $simigiftvoucher = Mage::getModel('simigiftvoucher/giftvoucher')
                    ->setData($giftcard)
                    ->setIncludeHistory(true)
                    ->save();
                $result[] = $simigiftvoucher->toArray();
    ...
    public function history($code)
    {
        ...
        if (!$giftVoucher->getId()) {
            $this->_fault('invalid_code', 
                Mage::helper('simigiftvoucher')->__('Gift card "%s" is not exists', $code));
        }
    ...
    public function update($code, $data)
    {
        ...
        try {
            ...
            $data['extra_content'] = Mage::helper('simigiftvoucher')->__('Updated by Api user');
    ...
    public function sendEmail($data)
    {
        ...
        if (!isset($data['gift_code']) || !isset($data['type'])) {
            $this->_fault('data_invalid', Mage::helper('simigiftvoucher')->__('Invalid data'));
        }
        $code = $data['gift_code'];
        ...
        if ($sendtofriend == 'to_friend') {
            if ($giftVoucher->getRecipientEmail()) {
                ...
                $this->_fault('data_invalid', 
                    Mage::helper('simigiftvoucher')->__('The email address of Gift Card recipient does not exist.'));
            }
            ...
            if ($giftVoucher->getCustomerEmail()) {
                ...
                $this->_fault('data_invalid', 
                    Mage::helper('simigiftvoucher')->__('Email of Gift card Purchaser does not exist.'));
            }
    ...
    public function redeemToCredit($customerId, $code)
    {
        ...
        if (!$customer->getEmail()) {
            $this->_fault('data_invalid', 
                Mage::helper('simigiftvoucher')->__('Customer ID does not exist.'));
        }

        if (!Mage::helper('simigiftvoucher')->canUseCode($giftVoucher)) {
            $this->_fault('limited_code', $code . 
                Mage::helper('simigiftvoucher')->__('This gift code limits the number of users'));
        }

        if ($giftVoucher->getBalance() == 0) {
            $this->_fault('invalid_code', $code . 
                Mage::helper('simigiftvoucher')->__(' - The current balance of this gift code is 0.'));
        }
        if ($giftVoucher->getStatus() != 2 && $giftVoucher->getStatus() != 4) {
            $this->_fault('invalid_code', $code . 
                Mage::helper('simigiftvoucher')->__('Gift card "%s" is not avaliable'));
        } else {
            $balance = $giftVoucher->getBalance();

            $credit = Mage::getModel('simigiftvoucher/credit')->getCreditByCustomerId($customerId);
            ...
            $history = Mage::getModel('simigiftvoucher/history')->setData(array(
                ...
                'extra_content' => Mage::helper('simigiftvoucher')->__('Redeemed by Api'),
            ));
            try {
                $credit->save();
            } catch (Exception $e) {
            ...
            try {
                ...
                $giftVoucher
                    ->setBalance($balance)
                    ->setStatus(Simi_Simigiftvoucher_Model_Status::STATUS_ACTIVE)->save();
                $credit->setBalance($credit->getBalance() - $amount)->save();
                $this->_fault('data_invalid', $e->getMessage());
            }
        }
    ...
    public function updateCredit($customerId, $balance, $currency = NULL)
    {
        $customer = Mage::getModel('customer/customer')->load($customerId);
        if (!$customer->getEmail()) {
            $this->_fault('data_invalid', Mage::helper('simigiftvoucher')->__('Customer ID does not exist.'));
        }
        $credit = Mage::getModel('simigiftvoucher/credit')->getCreditByCustomerId($customerId);
        if (!$currency) {
    ...
    public function processData($data)
    {
        if (!isset($data['template_name'])) {
            $data['template_name'] = Mage::helper('simigiftvoucher')->__('Created by Api user');
        }
        ...
        $data['extra_content'] = Mage::helper('simigiftvoucher')->__('Created by Api user');
        return $data;
    }
