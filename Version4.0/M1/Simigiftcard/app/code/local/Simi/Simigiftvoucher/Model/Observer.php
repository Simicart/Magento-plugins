 ...
 * @author      Simi Developer
 */
class Simi_Simigiftvoucher_Model_Observer
{
    ...
    public function couponPostAction($observer)
    {
        ...
        if ($giftVoucher->getId() && $giftVoucher->getStatus() == Simi_Simigiftvoucher_Model_Status::STATUS_ACTIVE
            ...
            if ($quote->getCouponCode() && !Mage::helper('simigiftvoucher')->getGeneralConfig('use_with_coupon')) {
                $session->addNotice(Mage::helper('simigiftvoucher')->__('A coupon code has been used. You cannot apply gift codes with the coupon to get discount.'));
            } else {
                $count = 0;
                ...
                if ($count == count($items)) {
                    $session->addNotice(Mage::helper('simigiftvoucher')->__('Gift Cards cannot be used to purchase Gift Card products'));
                } else {
                    $giftVoucher->addToSession($session);
                    $session->setSimiuseGiftCard(1);
                    $session->addSuccess(Mage::helper('simigiftvoucher')->__('Gift code "%s" was applied successfully.',
                        Mage::helper('simigiftvoucher')->getHiddenCode($giftVoucher->getGiftCode())));
                }
            }
            $action->setFlag('', Mage_Core_Controller_Varien_Action::FLAG_NO_DISPATCH, true);
            $action->getResponse()->setRedirect(Mage::getUrl('checkout/cart'));
        } else {
            if (!Mage::helper('simigiftvoucher')->getGeneralConfig('use_with_coupon')
                && ($session->getSimiuseGiftCreditAmount() > 0 || $session->getSimigiftVoucherDiscount() > 0)
            ) {
                if ($session->getSimiuseGiftCardCredit() && $session->getSimiuseGiftCard()) {
                    $session->addNotice(Mage::helper('simigiftvoucher')->__('You cannot apply a coupon code with either gift codes or Gift Card credit at once to get discount.'));
                } elseif ($session->getSimiuseGiftCard()) {
                    $session->addNotice(Mage::helper('simigiftvoucher')->__('The gift code(s) has been used. You cannot apply a coupon code with gift codes to get discount.'));
                } elseif ($session->getSimiuseGiftCardCredit()) {
                    $session->addNotice(Mage::helper('simigiftvoucher')->__('An amount in your Gift Card credit has been used. You cannot apply a coupon code with Gift Card credit to get discount.'));
                }
                $action->setFlag('', Mage_Core_Controller_Varien_Action::FLAG_NO_DISPATCH, true);
                $action->getResponse()->setRedirect(Mage::getUrl('checkout/cart'));
            }
     ...
     * @return Mage_Checkout_CartController
     */
    public function addCouponAction($observer)
    {
        $action = $observer->getEvent()->getControllerAction();
        $session = Mage::getSingleton('checkout/session');
        $quote = $session->getQuote();
        ...
        if ($giftVoucher->getId() && $giftVoucher->getStatus() == Simi_Simigiftvoucher_Model_Status::STATUS_ACTIVE
            ...
            if ($quote->getCouponCode() && !Mage::helper('simigiftvoucher')->getGeneralConfig('use_with_coupon')) {
                $error = true;
                $message = Mage::helper('simigiftvoucher')->__('A coupon code has been used. You cannot apply gift codes with the coupon to get discount.');
            } else {
                $count = 0;
                $items = $quote->getAllItems();
                foreach ($items as $item) {
                ...
                if ($count == count($items)) {
                    $error = true;
                    $message = Mage::helper('simigiftvoucher')->__('Gift Cards cannot be used to purchase Gift Card products');
                } else {
                    $giftVoucher->addToSession($session);
                    $session->setSimiuseGiftCard(1);
                    $quote->collectTotals()->save();
                    $usedGiftCode = true;
                    $message = Mage::helper('simigiftvoucher')->__('Gift code "%s" was applied successfully.',
                        Mage::helper('simigiftvoucher')->getHiddenCode($giftVoucher->getGiftCode()));
                }
            }
            $result = array(
            ...
            if (!Mage::helper('simigiftvoucher')->getGeneralConfig('use_with_coupon')
                ...
                if ($session->getSimiuseGiftCardCredit() && $session->getSimiuseGiftCard()) {
                    $error = true;
                    $message = Mage::helper('simigiftvoucher')->__('You cannot apply a coupon code with either gift codes or Gift Card credit at once to get discount.');
                } elseif ($session->getSimiuseGiftCard()) {
                    $error = true;
                    $message = Mage::helper('simigiftvoucher')->__('The gift code(s) has been used. You cannot apply a coupon code with gift codes to get discount.');
                } elseif ($session->getSimiuseGiftCardCredit()) {
                    $error = true;
                    $message = Mage::helper('simigiftvoucher')->__('An amount in your Gift Card credit has been used. You cannot apply a coupon code with Gift Card credit to get discount.');
                }
                $result = array(
                    'error' => $error,
                    'message' => $message,
    ...
    public function collectTotalsBefore($observer)
    {
        ...
        if ($codes = $session->getSimigiftCodes()) {
            ...
            $session->setSimibaseAmountUsed(implode(',', $codesArray));
        } else {
            $session->setSimibaseAmountUsed(null);
        }
        $session->setSimibaseGiftVoucherDiscount(0);
        $session->setSimigiftVoucherDiscount(0);
        $session->setSimiuseGiftCreditAmount(0);

        foreach ($quote->getAllAddresses() as $address) {

            $address->setSimigiftcardCreditAmount(0);
            $address->setSimibaseUseGiftCreditAmount(0);
            $address->setSimiuseGiftCreditAmount(0);

            $address->setSimibaseGiftVoucherDiscount(0);
            $address->setSimigiftVoucherDiscount(0);

            $address->setSimigiftvoucherBaseHiddenTaxAmount(0);
            $address->setSimigiftvoucherHiddenTaxAmount(0);
            ...
            foreach ($address->getAllItems() as $item) {
                ...
                if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                    foreach ($item->getChildren() as $child) {
                        $child->setSimibaseGiftVoucherDiscount(0)
                            ->setSimigiftVoucherDiscount(0)
                            ->setSimibaseUseGiftCreditAmount(0)
                            ->setSimibaseDiscount(0)
                            ->setSimiuseGiftCreditAmount(0);
                    }
                } elseif ($item->getProduct()) {
                    $item->setSimibaseGiftVoucherDiscount(0)
                        ->setSimigiftVoucherDiscount(0)
                        ->setSimibaseUseGiftCreditAmount(0)
                        ->setSimibaseDiscount(0)
                        ->setSimiuseGiftCreditAmount(0);
    ...
    public function orderPlaceBefore($observer)
    {
        ...
        if ($codes = $session->getSimigiftCodes()) {
            ...
            foreach ($SimibaseAmountUsed as $code => $amount) {
                $model = Mage::getModel('simigiftvoucher/giftvoucher')->loadByCode(strval($code));
                if (!$model || $model->getStatus() != Simi_Simigiftvoucher_Model_Status::STATUS_ACTIVE
    ...
    public function orderPlaceAfter($observer)
    {
        ...
        if ($session->getMessageApplyGiftcardWithCouponCode()) {
            ...
            Mage::throwException(Mage::helper('simigiftvoucher')->__('A coupon code has been used. You cannot apply gift codes or Gift Card credit with the coupon to get discount.'));
        }
        ...
        if (!$session->getSimiuseGiftCard() && !($session->getSimiuseGiftCardCredit())) {
            return;
        }
        //zend_debug::dump($order->debug());
        //zend_debug::dump($order->getSimigiftCodes());die(2);

        if ($codes = $order->getSimigiftCodes()) {

            $order->setGiftvoucherForOrderCodes($codes)
                ->setGiftvoucherForOrderAmount($order->getSimigiftVoucherDiscount());
            //zend_debug::dump($order->getGiftvoucherForOrderCodes());
            $codesArray = explode(',', $codes);
            $codesBaseDiscount = explode(',', $order->getSimicodesBaseDiscount());
            $codesDiscount = explode(',', $order->getSimicodesDiscount());
            ...
            foreach ($codesArray as $code) {
                ...
                $history = Mage::getModel('simigiftvoucher/history')->setData(array(
                    ...
                    'created_at' => now(),
                    'action' => Simi_Simigiftvoucher_Model_Actions::ACTIONS_SPEND_ORDER,
                    'amount' => $codeCurrentDiscount,
                    'balance' => $currentBalance,
                    'currency' => $store->getCurrentCurrencyCode(),
                    'status' => $giftVoucher->getStatus(),
                    'order_amount' => $discount[$code],
                    'comments' => Mage::helper('simigiftvoucher')->__('Spent on order %s', $order->getIncrementId()),
                    'extra_content' => Mage::helper('simigiftvoucher')->__('Used by %s %s',
                ...
                if ($order->getCustomerId()) {
                    ...
                    if (!$collection->getSize()) {
                        try {
                            ...
                            Mage::getModel('simigiftvoucher/customervoucher')
        ...
        if ($order->getSimigiftcardCreditAmount() && $order->getCustomerId()) {
            ...
            if ($credit->getId()) {
                try {
                    ...
                    $credithistory = Mage::getModel('simigiftvoucher/credithistory')->setData($credit->getData());
                    $credithistory->addData(array(
                        'action' => 'Spend',
                        'currency_balance' => $currencyBalance,
                        'order_id' => $order->getId(),
                        'order_number' => $order->getIncrementId(),
                        'balance_change' => $order->getSimiuseGiftCreditAmount(),
                        'created_date' => now(),
                        'currency' => $store->getCurrentCurrencyCode(),
                        'base_amount' => $order->getSimibaseUseGiftCreditAmount(),
                        'amount' => $order->getSimiuseGiftCreditAmount()
                    ))->setId(null)->save();
                } catch (Exception $e) {
                    Mage::logException($e);
                }
    ...
    {
        $giftVouchers = Mage::getModel('simigiftvoucher/history')->getCollection()->joinGiftVoucher()
        ...
        if ($baseDiscount) {
            ...
            $order->setSimigiftCodes(implode(',', $codesArray));
            $order->setSimibaseGiftVoucherDiscount($baseDiscount);
            $order->setSimigiftVoucherDiscount($discount);
        }
        $creditHistory = Mage::getResourceModel('simigiftvoucher/credithistory_collection')
            ->addFieldToFilter('action', 'Spend')
            ->addFieldToFilter('order_id', $order->getId())
            ->getFirstItem();
        if ($creditHistory && $creditHistory->getId()) {
            $order->setSimigiftcardCreditAmount($creditHistory->getBalanceChange());
            $order->setSimibaseUseGiftCreditAmount($creditHistory->getBaseAmount());
            $order->setSimiuseGiftCreditAmount($creditHistory->getAmount());
        }
        return $this;
    }
    ...
    public function paypalPrepareItems($observer)
    {
        ...
        if (version_compare(Mage::getVersion(), '1.4.2', '>=')) {
            ...
            if ($paypalCart) {
                ...
                $giftCardDiscount = $salesEntity->getSimigiftVoucherDiscount();
                if (!$giftCardDiscount) {
                    $giftCardDiscount = $session->getSimigiftVoucherDiscount();
                }
                if ($giftCardDiscount) {
                    $paypalCart->updateTotal(Mage_Paypal_Model_Cart::TOTAL_DISCOUNT, abs((float)$giftCardDiscount),
                        Mage::helper('simigiftvoucher')->__('Gift Card Discount'));
                }

                $giftCardCreditAmount = $salesEntity->getSimiuseGiftCreditAmount();
                if (!$giftCardCreditAmount) {
                    $giftCardCreditAmount = $session->getSimiuseGiftCreditAmount();
                }
                if ($giftCardCreditAmount && Mage::helper('simigiftvoucher')->getGeneralConfig('enablecredit')) {
                    $paypalCart->updateTotal(Mage_Paypal_Model_Cart::TOTAL_DISCOUNT,
                        abs((float)$giftCardCreditAmount), Mage::helper('simigiftvoucher')->__('Gift Card Credit'));
                }
            }
        } else {
            ...
            if ($salesEntity && $additional) {
                ...
                $items[] = new Varien_Object(array(
                    'name' => Mage::helper('simigiftvoucher')->__('Gift Card Discount'),
                    'qty' => 1,
                    'amount' => -(abs((float)$salesEntity->getSimigiftVoucherDiscount())),
                ));
                if (Mage::helper('simigiftvoucher')->getGeneralConfig('enablecredit')) {
                    $items[] = new Varien_Object(array(
                        'name' => Mage::helper('simigiftvoucher')->__('Gift Card Credit'),
                        'qty' => 1,
                        'amount' => -(abs((float)$salesEntity->getSimiuseGiftCreditAmount())),
                    ));
                }
    ...
    public function orderLoadAfter($observer)
    {
        ...
        if ((abs($order->getSimigiftVoucherDiscount()) < 0.0001 && abs($order->getSimiuseGiftCreditAmount()) < 0.0001)
        ...
        foreach ($order->getAllItems() as $item) {
            ...
            } elseif ($item->getSimigiftVoucherDiscount() || $item->getSimiuseGiftCreditAmount()) {
    ...
    public function creditmemoRegisterBefore($observer)
    {
        ...
        if (($order->getSimigiftVoucherDiscount() > 0 || $order->getSimiuseGiftCreditAmount() > 0)
        ...
        if (isset($input['giftcard_refund'])) {
            ...
            $maxAmount = 0;
            if ($creditmemo->getSimiuseGiftCreditAmount()
                && Mage::helper('simigiftvoucher')->getGeneralConfig('enablecredit', $creditmemo->getStoreId())
            ) {
                $maxAmount += floatval($creditmemo->getSimiuseGiftCreditAmount());
            }
            if ($creditmemo->getSimigiftVoucherDiscount()) {
                $maxAmount += floatval($creditmemo->getSimigiftVoucherDiscount());
            }
    ...
    public function invoiceSaveAfter($observer)
    {
        ...
        foreach ($invoice->getAllItems() as $itemCredit) {
            ...
            if (isset($item) && $item != null) {
                ...
                $giftVouchers = Mage::getModel('simigiftvoucher/giftvoucher')
                    ->getCollection()->addItemFilter($item->getId());
                $itemQtyInvoice = $itemCredit->getQty();
                foreach ($giftVouchers as $giftVoucher) {
                    if ($giftVoucher->getStatus() == Simi_Simigiftvoucher_Model_Status::STATUS_PENDING) {
                        $giftVoucher->addData(array(
                            'status' => Simi_Simigiftvoucher_Model_Status::STATUS_ACTIVE,
                            'comments' => Mage::helper('simigiftvoucher')->__('Active when order is complete'),
                            'amount' => $giftVoucher->getBalance(),
                            'action' => Simi_Simigiftvoucher_Model_Actions::ACTIONS_UPDATE,
                        ...
                        try {
                            ...
                            if (!empty($buyRequest['recipient_ship'])) {
                                ...
                                if (!Mage::helper('simigiftvoucher')->getEmailConfig('send_with_ship',
                                    $order->getStoreId())
                                ) {
                                    $giftVoucher->setData('dont_send_email_to_recipient', 1);
                                }
                            ...
                            $giftVoucher->save();
                            if (Mage::helper('simigiftvoucher')->getEmailConfig('enable', $order->getStoreId())) {
                                //Hai.Tran
    ...
    public function creditmemoSaveAfter($observer)
    {
        ...
        foreach ($creditmemo->getAllItems() as $itemCredit) {
            ...
            if (isset($item) && $item != null) {
                ...
                foreach ($giftVouchers as $giftVoucher) {
                    ...
                    if (
                        (
                            ...
                            $giftVoucher->getStatus() == Simi_Simigiftvoucher_Model_Status::STATUS_DISABLED ||
                            $giftVoucher->getStatus() == Simi_Simigiftvoucher_Model_Status::STATUS_EXPIRED ||
                            $giftVoucher->getStatus() == Simi_Simigiftvoucher_Model_Status::STATUS_DELETED
                        )
                ...
                if ($cantRefundGiftvoucherProduct < ($item->getQtyInvoiced() - $item->getQtyRefunded())) {
                    Mage::getSingleton('adminhtml/session')->addError(Mage::helper('simigiftvoucher')->__('There is atleast one of products which is simigiftvoucher and being used.'));
                    throw new Exception(
                        Mage::helper('simigiftvoucher')->__('There is atleast one of products which is simigiftvoucher and being used.')
                    );
                }
                ...
                foreach ($giftVouchers as $giftVoucher) {
                    if ($giftVoucher->getStatus() == Simi_Simigiftvoucher_Model_Status::STATUS_ACTIVE
                        ...
                        $itemQtyRefund -= 1;
                        $giftVoucher->addData(array(
                            'status' => Simi_Simigiftvoucher_Model_Status::STATUS_DISABLED,
                            'comments' => Mage::helper('simigiftvoucher')->__('Refund order %s', $order->getIncrementId()),
                            'amount' => $giftVoucher->getBalance(),
                            'action' => Simi_Simigiftvoucher_Model_Actions::ACTIONS_REFUND,
                            'order_increment_id' => $order->getIncrementId(),
                        ...
                        try {
                            ...
                            if ($giftVoucher->getData('is_sent')
                                && Mage::helper('simigiftvoucher')->getEmailConfig('send_refund', $order->getStoreId())
                            ) {
        ...
        if (Mage::app()->getStore()->isAdmin() && $creditmemo->getGiftcardRefundAmount()) {
            ...
            if ($customer->getId()
                ...
                if (
                    !Mage::helper('simigiftvoucher')->getGeneralConfig('credit_condition', $creditmemo->getStoreId())
                    && $this->_hasGiftcodeCondition($order)
                ) {
                    $refundAmount = 0;
                    ...
                    $this->refundToCredit($order, $creditmemo->getGiftcardRefundAmount());
                }
            } else {
                $refundAmount = 0;
                $baseCurrency = Mage::app()->getStore($order->getStoreId())->getBaseCurrency();
        ...
        if (!Mage::app()->getStore()->isAdmin() && Mage::helper('simigiftvoucher')->getGeneralConfig('online_refund')) {
            if ($creditmemo->getSimibaseGiftVoucherDiscount()) {
                $maxAmount = floatval($creditmemo->getSimibaseGiftVoucherDiscount());
    ...
    protected function _refundOffline($order, $baseGrandTotal)
    {
        ...
        if ($codes = $order->getSimigiftCodes()) {
            ...
            foreach ($codesArray as $code) {
                ...
                $giftVoucher = Mage::getModel('simigiftvoucher/giftvoucher')->loadByCode($code, $order->getStoreId());
                ...
                if ($giftVoucher->getStatus() == Simi_Simigiftvoucher_Model_Status::STATUS_USED) {
                    $giftVoucher->setStatus(Simi_Simigiftvoucher_Model_Status::STATUS_ACTIVE);
                }
                $giftVoucher->setData('balance', $balance)->save();
                ...
                $history->setData(array(
                    ...
                    'status' => $giftVoucher->getStatus(),
                    'comments' => Mage::helper('simigiftvoucher')->__('Refund from order %s', $order->getIncrementId()),
                    'customer_id' => $order->getData('customer_id'),
                    'customer_email' => $order->getData('customer_email'),
                ))->save();
            }
        ...
        if ($order->getSimigiftcardCreditAmount() && $order->getCustomerId()
            ...
            if ($credit->getId()) {
                ...
                $histories = Mage::getResourceModel('simigiftvoucher/credithistory_collection')
                    ->addFieldToFilter('customer_id', $order->getCustomerId())
    ...
    public function orderSaveAfter($observer)
    {
        ...
        if (in_array($order->getStatus(), $refundState)) {
            $this->_refundOffline($order, $order->getSimibaseGiftVoucherDiscount());
            ...
            foreach ($order->getAllItems() as $item) {
                ...
                $req = $productOptions['info_buyRequest'];
                ...
                foreach ($giftVouchers as $giftVoucher) {
                    ...
                    if ($giftVoucher->getUsed() == Simi_Simigiftvoucher_Model_Giftvoucher::STATUS_YES) {
                        $giftVoucher->addData(array(
                            'used' => Simi_Simigiftvoucher_Model_Giftvoucher::STATUS_NO,
                            'status' => Simi_Simigiftvoucher_Model_Status::STATUS_DISABLED,
                            'comments' => Mage::helper('simigiftvoucher')->__('Canceled order'),
                            'amount' => $giftVoucher->getBalance(),
                            'action' => Simi_Simigiftvoucher_Model_Actions::ACTIONS_UPDATE,
                            'order_increment_id' => $order->getIncrementId()
                        ))->setIncludeHistory(true);
    ...
    protected function _addGiftVoucherForOrder($order)
    {
        ...
        foreach ($order->getAllItems() as $item) {
            ...
            for ($i = 0; $i < $item->getQtyOrdered() - $giftVouchers->getSize(); $i++) {
                ...
                $giftCodeSets = Mage::getModel('simigiftvoucher/giftvoucher')
                    ->getCollection()->addFieldToFilter('set_id', $product->getSimigiftCodeSets())
                    ->addFieldToFilter('used',
                        Simi_Simigiftvoucher_Model_Giftvoucher::STATUS_NO)->getFirstItem()->getGiftCode();

                if ($giftCodeSets) {
                    $giftVoucher = Mage::getModel('simigiftvoucher/giftvoucher')
                        ->getCollection()->addFieldToFilter('set_id', $product->getSimigiftCodeSets())
                        ->addFieldToFilter('used', Simi_Simigiftvoucher_Model_Giftvoucher::STATUS_NO)->getFirstItem();
                } else {
                    $giftVoucher = Mage::getModel('simigiftvoucher/giftvoucher');
                }
                ...
                $giftProduct = Mage::getModel('simigiftvoucher/product')->loadByProduct($product);
                ...
                if (isset($buyRequest['giftcard_template_image']) && $buyRequest['giftcard_template_image']) {
                    if (isset($buyRequest['giftcard_use_custom_image']) && $buyRequest['giftcard_use_custom_image']) {
                        $dir = Mage::getBaseDir('media') . DS . 'tmp' . DS . 'simigiftvoucher' . DS . 'images' .
                        ...
                        if (file_exists($dir)) {
                            ...
                            $imagePath = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) .
                                'simigiftvoucher/template/images/';
                ...
                if ($order->getStatus() == Mage_Sales_Model_Order::STATE_COMPLETE) {
                    ...
                    $giftVoucher->setStatus(Simi_Simigiftvoucher_Model_Status::STATUS_PENDING);
                }

                if ($timeLife = Mage::helper('simigiftvoucher')->getGeneralConfig('expire', $order->getStoreId())) {
                ...
                $giftVoucher->setAction(Simi_Simigiftvoucher_Model_Actions::ACTIONS_CREATE)
                    ->setComments(Mage::helper('simigiftvoucher')->__('Created for order %s', $order->getIncrementId()))
                    ->setOrderIncrementId($order->getIncrementId())
                    ->setOrderItemId($item->getId())
                    ->setExtraContent(Mage::helper('simigiftvoucher')->__('Created by customer %s %s',
                        $order->getData('customer_firstname'), $order->getData('customer_lastname')))
                    ->setIncludeHistory(true);
                try {
                    if ($giftVoucher->getDayToSend() && strtotime($giftVoucher->getDayToSend()) > time()
                    ...
                    if (!empty($buyRequest['recipient_ship'])) {
                        $giftVoucher->setData('is_sent', 2);
                        if (!Mage::helper('simigiftvoucher')->getEmailConfig('send_with_ship', $order->getStoreId())) {
                            $giftVoucher->setData('dont_send_email_to_recipient', 1);
                        }
                    }
                    ...
                    if ($giftCodeSets) {
                        ...
                        $giftVoucher->setUsed(Simi_Simigiftvoucher_Model_Giftvoucher::STATUS_YES);
                    }

                    //  die(now(true));
                    $giftVoucher->save();
                    $allowStatus = explode(',',
                        Mage::helper('simigiftvoucher')->getEmailConfig('only_complete', $order->getStoreId()));
                    if (!is_array($allowStatus)) {
                        $allowStatus = array();
                    }
                    if (in_array($giftVoucher->getStatus(), $allowStatus)) {
                        if ($giftVoucher->getRecipientEmail()) {
                            $giftVoucher->sendEmailToRecipient();
                        } else {
                            $giftVoucher->sendEmail();
                        }
                    }
    ...
    {
        if (Mage::helper('simigiftvoucher')->getEmailConfig('autosend')) {
            $giftVouchers = Mage::getModel('simigiftvoucher/giftvoucher')->getCollection()
                ->addFieldToFilter('status', array('neq' => Simi_Simigiftvoucher_Model_Status::STATUS_DELETED))
                ->addExpireAfterDaysFilter(Mage::helper('simigiftvoucher')->getEmailConfig('daybefore'));
            foreach ($giftVouchers as $giftVoucher) {
                $giftVoucher->sendEmail();
            }
        }
    ...
    {
        $collection = Mage::getResourceModel('simigiftvoucher/giftvoucher_collection');
        $timeSite = date("Y-m-d H:i:s", Mage::getModel('core/date')->timestamp(time()));
    ...
    public function adminhtmlCatalogProductSaveAfter($observer)
    {
        ...
        $session = Mage::getSingleton('simigiftvoucher/session');
        ...
        $url = Mage::getModel('adminhtml/url')->getUrl("adminhtml/simigiftvoucher_giftproduct/index");
    ...
    public function giftcardPaymentMethod($observer)
    {
        ...
        if (Mage::helper('simigiftvoucher')->getGeneralConfig('active', $storeId)) {
            ...
            if (($block instanceof Mage_Checkout_Block_Onepage_Payment_Methods)) {
                ...
                // @ to fix the error on the developer mode
                $htmlAddgiftcard = @$block->getLayout()->createBlock('simigiftvoucher/payment_form')->renderView();
            ...
            if (($block instanceof Simi_RewardPoints_Block_Checkout_Cart_Rewrite_Coupon)
                ...
                $transport = $observer['transport'];
                $html = $transport->getHtml();
                $htmlAddgiftcardform = $block->getLayout()->createBlock('simigiftvoucher/cart_giftcard')->renderView();
    ...
    public function showLinks($observer)
    {
        ...
        if (Mage::helper('simigiftvoucher')->getGeneralConfig('active', $storeId)) {
            ...
            if (($block instanceof Mage_Page_Block_Template_Links)
                && Mage::helper('simigiftvoucher')->getGeneralConfig('gc_show_check_link')
            ) {
                if (!Mage::registry('show_check_giftcard_link')) {
                    ...
                    if (isset($topLinks) && $topLinks != null) {
                        $topLinks->addLink('Check Gift Card',
                            Mage::helper('simigiftvoucher')->getCheckGiftCardUrl(), 'Check Gift Card', false, null, 10);
                    }
                }
            }
        }
    ...
    public function updateShippedGiftCard($observer)
    {
        ...
        $giftVouchers = Mage::getResourceModel('simigiftvoucher/giftvoucher_collection')->addItemFilter($orderItemId);
        foreach ($giftVouchers as $giftCard) {
            if ($giftCard->getShippedToCustomer()
                || !Mage::getStoreConfig('simigiftvoucher/general/auto_shipping', $giftCard->getStoreId())
            ) {
                return;
            }
            try {
    ...
    public function customerSaveAfter($observer)
    {
        ...
        $credit = Mage::getModel('simigiftvoucher/credit')->getCreditByCustomerId($customer->getId());
        ...
        try {
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('simigiftvoucher')->__($e->getMessage()));
        }
        try {
            $credithistory->save();
        } catch (Mage_Core_Exception $e) {
    ...
    {
        $product = $observer->getEvent()->getProduct();
        if ($product->getTypeId() != 'simigiftvoucher' || !$product->getId()) {
            return $this;
        }
        $model = Mage::getSingleton('simigiftvoucher/product');
    ...
    {
        $session = Mage::getSingleton('checkout/session');
        if ($session->getQuote()->getCouponCode() && !Mage::helper('simigiftvoucher')->getGeneralConfig('use_with_coupon')
            && ($session->getSimiuseGiftCreditAmount() > 0 || $session->getSimigiftVoucherDiscount() > 0)
        ) {
            $session->addNotice(Mage::helper('simigiftvoucher')->__('A coupon code has been used. You cannot apply gift codes or Gift Card credit with the coupon to get discount.'));
            $session->setMessageApplyGiftcardWithCouponCode(false);
        }
    }
    ...
    public function predispatchCheckout($observer)
    {
        ...
        if ($requestPath == 'checkout_multishipping_addresses') {
            $cart->setSimiuseGiftCard(0)
                ->setSimiuseGiftCardCredit(0);
        }
        ...
        foreach ($items as $item) {
            ...
            if (isset($data['value']) && $data['value'] != null) {
                $session->addNotice(Mage::helper('simigiftvoucher')->__('You need to add your friend\'s address as the shipping address. We will send this gift card to that address.'));
                return $this;
            }
        }

        if ($cart->getQuote()->getCouponCode() && !Mage::helper('simigiftvoucher')->getGeneralConfig('use_with_coupon')
            && ($cart->getSimiuseGiftCreditAmount() > 0 || $cart->getSimigiftVoucherDiscount() > 0)
        ) {
            $session->setMessageApplyGiftcardWithCouponCode(false);
            $session->addNotice(Mage::helper('simigiftvoucher')->__('A coupon code has been used. You cannot apply gift codes or Gift Card credit with the coupon to get discount.'));
        }
    ...
    public function controllerActionLayoutGenerateBlocksAfter($observer)
    {
        ...
        if (Mage::helper('simigiftvoucher')->getGeneralConfig('active', $storeId)) {
            ...
            if (isset($class) && $class != null) {
                ...
                if (strpos($extensionName, 'Mage_') === false && $extensionName != 'Simi_Onestepcheckout') {
    ...
    {
        $dir = Mage::getBaseDir('media') . DS . 'tmp' . DS . 'simigiftvoucher' . DS . 'cache' . DS;
        $this->delTree($dir);
        return;
    }
    ...
    {
        $dir = Mage::getBaseDir('media') . DS . 'tmp' . DS . 'simigiftvoucher' . DS . 'images' . DS;
        $h = opendir($dir);
    ...
    public function applyWithCoupon($observer)
    {
        ...
        if (isset($params['order']['coupon']['code']) && $params['order']['coupon']['code'] != null) {
            $giftVoucher = Mage::getModel('simigiftvoucher/giftvoucher')->loadByCode($params['order']['coupon']['code']);
            ...
            if ($giftVoucher->getId() && $giftVoucher->getStatus() == Simi_Simigiftvoucher_Model_Status::STATUS_ACTIVE
                ...
                if ($count == count($items)) {
                    $adminSession->addNotice(Mage::helper('simigiftvoucher')->__('Gift Cards cannot be used to purchase Gift Card products'));
                ...
                if (!Mage::helper('simigiftvoucher')->getGeneralConfig('use_with_coupon')
                    ...
                    if ($session->getSimiuseGiftCardCredit() && $session->getSimiuseGiftCard()) {
                        $adminSession->addNotice(Mage::helper('simigiftvoucher')->__('You cannot apply a coupon code with either gift codes or Gift Card credit at once to get discount.'));
                    } elseif ($session->getSimiuseGiftCard()) {
                        $adminSession->addNotice(Mage::helper('simigiftvoucher')->__('The gift code(s) has been used. You cannot apply a coupon code with gift codes to get discount.'));
                    } elseif ($session->getSimiuseGiftCardCredit()) {
                        $adminSession->addNotice(Mage::helper('simigiftvoucher')->__('An amount in your Gift Card credit has been used. You cannot apply a coupon code with Gift Card credit to get discount.'));
                    }
                    unset($params['order']['coupon']);
    ...
    public function removeItemsLayout($extensionName)
    {
        ...
        if (isset($headBlock) && $headBlock != null) {
            $jsUrl = Mage::getBaseDir() . DS . 'js' . DS . 'Simi' . DS . 'simigiftvoucher' . DS . 'onestepcheckout' .
                DS . $extensionName . DS . 'simigiftvoucher.js';
            $cssUrl1 = str_replace(Mage::getBaseUrl(), Mage::getBaseDir() . DS,
                Mage::getDesign()->getSkinUrl('css/Simi/onestepcheckout/' .
                    $extensionName . '/simigiftvoucher.css', array()));
            ...
            if (is_file($jsUrl)) {
                $headBlock->removeItem('js', 'Simi/simigiftvoucher/giftvoucher.js');
                $headBlock->addJs('Simi/simigiftvoucher/onestepcheckout/' . $extensionName . '/simigiftvoucher.js');
            }
            if (is_file(str_replace('/', DIRECTORY_SEPARATOR, $cssUrl1))) {
                $headBlock->removeItem('skin_css', 'css/Simi/simigiftvoucher.css');
                $headBlock->addCss('css/Simi/onestepcheckout/' . $extensionName . '/simigiftvoucher.css');
            }

            if (is_file(str_replace('/', DIRECTORY_SEPARATOR, $cssUrl2))) {
                $headBlock->removeItem('skin_css', 'css/Simi/reupdate.css');
                $headBlock->addCss('css/Simi/onestepcheckout/' . $extensionName . '/reupdate.css');
            }

            if (is_file(str_replace('/', DIRECTORY_SEPARATOR, $cssUrl3))) {
                $headBlock->removeItem('skin_css', 'css/Simi/reupdate_temp3.css');
                $headBlock->addCss('css/Simi/onestepcheckout/' . $extensionName . '/reupdate_temp3.css');
            }
            if (is_file(str_replace('/', DIRECTORY_SEPARATOR, $cssUrl4))) {
                $headBlock->removeItem('skin_css', 'css/Simi/mobile_giftvoucher.css');
                $headBlock->addCss('css/Simi/onestepcheckout/' . $extensionName . '/mobile_giftvoucher.css');
            }
        }
    }
    ...
    function prepareLayoutBefore($observer)
    {
        ...
        if ("head" == $block->getNameInLayout()) {
            $file = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_JS) . '/Simi/simigiftvoucher/jquery-1.11.2.min.js';
            $block->addJs($file);
        }
        return $this;
    }
     ...
     * @param $amount
     */
    protected function refundToCredit($order, $amount)
    {
        $customer = Mage::getModel('customer/customer')->load($order->getCustomerId());
     ...
     * @return bool
     */
    protected function _hasGiftcodeCondition($order)
    {
        if ($codes = $order->getSimigiftCodes()) {
            $codesArray = explode(',', $codes);
            foreach ($codesArray as $code) {
                /** @var Simi_Simigiftvoucher_Model_Giftvoucher $giftVoucher */
                $giftVoucher = Mage::getModel('simigiftvoucher/giftvoucher')->loadByCode($code, $order->getStoreId());
                $conditions = unserialize($giftVoucher->getConditionsSerialized());
                $actions = unserialize($giftVoucher->getActionsSerialized());
                if ($conditions['conditions'] || $actions['conditions']) {
                    return true;
                }
            }
        }
