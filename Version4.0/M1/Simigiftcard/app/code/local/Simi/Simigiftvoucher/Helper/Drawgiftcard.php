...
class Simi_Simigiftvoucher_Helper_Drawgiftcard extends Mage_Core_Helper_Data
{
    ...
    {
        $gcTemplateDir = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'draw' . DS . $giftcode . DS;
    ...
    public function draw($giftcode)
    {
        ...
        if (isset($giftcode['giftcard_template_id']) && $giftcode['giftcard_template_id'] != null) {
            $giftcardTemplate = Mage::getModel('simigiftvoucher/gifttemplate')->load($giftcode['giftcard_template_id']);
    ...
    public function generateLeftImage($giftcode, $giftcardTemplate)
    {
        ...
        $y = 30;
        $fsize = 15;
        $font = Mage::getBaseDir('lib') . DS . 'Simi' . DS . 'fonts' . DS . 'OpenSans-Semibold.ttf';
        ...
        /* Draw Expiry Date */
        if (Mage::helper('simigiftvoucher')->getGeneralConfig('show_expiry_date')) {
            if ($giftcode['expired_at']) {
                $expiryDate = Mage::helper('simigiftvoucher')->__('Expired: ') . 
        ...
        // $font = Mage::getBaseDir('lib') . DS . 'Simi' . DS . 'fonts' . DS . 'OpenSans-Light.ttf';
        /* Print "From:" and "To: " */

        $textbox = imageftbbox($fsize, 0, $font, Mage::helper('simigiftvoucher')->__('From: '));
        imagefttext($img1, 13, 0, 15, $y, $textColor, $font, Mage::helper('simigiftvoucher')->__('From: '));
        imagefttext($img1, 13, 0, $x + ($textbox[2] - $textbox[0]), $y, $styleColor, $font, 
            $giftcode['customer_name']);
        $y -= 1.55 * ($textbox[7] - $textbox[1]);

        $textbox = imageftbbox($fsize, 0, $font, Mage::helper('simigiftvoucher')->__('To: '));
        imagefttext($img1, 13, 0, 15, $y + 5, $textColor, $font, Mage::helper('simigiftvoucher')->__('To: '));
        ...
        $textbox = imageftbbox($fsize, 0, $font, Mage::helper('simigiftvoucher')->__('Value '));
        imagefttext($img1, 10, 0, 15, $valueY, $textColor, $font, Mage::helper('simigiftvoucher')->__('Value '));
        ...
        $fontCode = Mage::getBaseDir('lib') . DS . 'Simi' . DS . 'fonts' . DS . 'OpenSans-SemiboldItalic.ttf';
        $codeY = $y + 105;
        ...
        if (isset($giftcardTemplate['notes']) && $giftcardTemplate['notes'] != null) {
            ...
            $notes = Mage::getStoreConfig('simigiftvoucher/print_voucher/note', $storeId);
    ...
    public function generateSimpleImage($giftcode, $giftcardTemplate)
    {
        ...
        /* Print Gift Code */
        $fontCode = Mage::getBaseDir('lib') . DS . 'Simi' . DS . 'fonts' . DS . 'OpenSans-Semibold.ttf';
        $fsizeCode = 15;
    ...
    public function generateTopImage($giftcode, $giftcardTemplate)
    {
        ...
        /* Print Value */
        $fontPrice = Mage::getBaseDir('lib') . DS . 'Simi' . DS . 'fonts' . DS . 'OpenSans-ExtraBold.ttf';
        $fsizePrice = 14;
        ...
        $fsizeValue = 9;
        $textbox = imageftbbox($fsizeValue, 0, $fontPrice, Mage::helper('simigiftvoucher')->__('Value '));
        $valueX += $textbox[2] - $textbox[0] + 15;
        imagefttext($img6, $fsizeValue, 0, 210 - $valueX, $y, $styleColor, $font, 
            Mage::helper('simigiftvoucher')->__('Value '));

        /* Print "From:" and "To: " */

        $textbox = imageftbbox($fsize, 0, $font, Mage::helper('simigiftvoucher')->__('From: '));
        imagefttext($img1, 13, 0, 15, $y - 5, $textColor, $font, Mage::helper('simigiftvoucher')->__('From: '));
        imagefttext($img1, 13, 0, $x + ($textbox[2] - $textbox[0]), $y - 5, $textColor, $font, 
            $giftcode['customer_name']);
        $y -= 1.55 * ($textbox[7] - $textbox[1]);

        $textbox = imageftbbox($fsize, 0, $font, Mage::helper('simigiftvoucher')->__('To: '));
        imagefttext($img1, 13, 0, 15, $y, $textColor, $font, Mage::helper('simigiftvoucher')->__('To: '));
    ...
    public function generateCenterImage($giftcode, $giftcardTemplate)
    {
        ...
        $y += 135;
        $textbox = imageftbbox($fsize, 0, $font, Mage::helper('simigiftvoucher')->__('From: '));
        imagefttext($img2, 13, 0, 15, $y - 5, $textColor, $font, Mage::helper('simigiftvoucher')->__('From: '));
        imagefttext($img2, 13, 0, $x + ($textbox[2] - $textbox[0]), $y - 5, $styleColor, $font, 
            $giftcode['customer_name']);
        $y -= 1.55 * ($textbox[7] - $textbox[1]);

        $textbox = imageftbbox($fsize, 0, $font, Mage::helper('simigiftvoucher')->__('To: '));
        imagefttext($img2, 13, 0, 15, $y, $textColor, $font, Mage::helper('simigiftvoucher')->__('To: '));
    ...
    public function createGCImage($filename, $type = null)
    {
        if (isset($type) && $type != null) {
            $dir = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'template' . DS . 'images' . DS . 
                $type . DS . $filename;
        } else {
            $dir = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'template' . DS . 'images' . DS . $filename;
        }

        if (($filename == null) || (!file_exists($dir))) {
            $dir = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'draw' . DS . 'default.png';
        }
    ...
    public function createGCBackground($filename, $type = null, $giftcode = null)
    {
        if ($filename) {
            if (isset($type) && $type != null) {
                $dir = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'template' . DS . 'background' . 
                    DS . $type . DS . $filename;
            } else {
                $dir = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'template' . DS . 'background' . 
                    DS . $filename;
    ...
    {
        $storeId = Mage::app()->getStore()->getId();
        $image = Mage::getStoreConfig('simigiftvoucher/print_voucher/logo', $storeId);
        if ($image) {
            $dir = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'pdf' . DS . 'logo' . 
                DS . str_replace("/", DS, $image);
            $imgLogo = $this->imagecreatefromfile($dir);
            
            if ($type == 'simple') {
                $ratio = imagesx($imgLogo) / imagesy($imgLogo);  
                $resizeLogoUrl = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'draw' . DS . 'logo' . 
    ...
    {
        if (isset($type) && $type != null) {
            $dir = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'template' . DS . 'messagebox' . 
    ...
    {
        $barcode = Mage::helper('simigiftvoucher')->getGeneralConfig('barcode_enable');
        $barcodeType = Mage::helper('simigiftvoucher')->getGeneralConfig('barcode_type');

        if ($barcodeType == 'code128') {
            $barcodeUrl = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'template' . DS . 'barcode' . 
                DS . $code . '.png';

            $resizeBarcodeUrl = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'draw' . DS . $code . 
                DS . 'barcode.png';
            ...
            $qr = new Simi_Simigiftvoucher_QRCode($code);
            $content = file_get_contents($qr->getResult());
            $fileName = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'draw' . DS . $code . DS . 'qrcode.png';
    ...
    {
        $directory = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'draw' . DS . $code . DS;
     ...
     * @return string
     */
    public function getBarcodeInFolder($code)
    {
        $directory = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'draw' . DS . $code . DS;
        return glob($directory . "barcode.png");
    }
