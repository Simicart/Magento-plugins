 ...
 * @author   Simi Developer
 */
class Simi_Simigiftvoucher_Adminhtml_Simigiftvoucher_GenerategiftcardController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function exportCsvAction()
    {
        ...
        $content = $this->getLayout()->createBlock('simigiftvoucher/adminhtml_generategiftcard_grid')->getCsv();
    ...
    public function exportXmlAction()
    {
        ...
        $content = $this->getLayout()->createBlock('simigiftvoucher/adminhtml_generategiftcard_grid')->getXml();
        $this->_prepareDownloadResponse($fileName, $content);
    }
    ...
    public function exportGiftCodeCsvAction()
    {
        ...
        $content = $this->getLayout()
            ->createBlock('simigiftvoucher/adminhtml_generategiftcard_edit_tab_giftcodelist')->getCsv();
        $this->_prepareDownloadResponse($fileName, $content);
    }
    ...
    public function exportGiftCodeXmlAction()
    {
        ...
        $content = $this->getLayout()
            ->createBlock('simigiftvoucher/adminhtml_generategiftcard_edit_tab_giftcodelist')->getXml();
        $this->_prepareDownloadResponse($fileName, $content);
    }
    ...
    public function exportGiftCodePdfAction()
    {
        ...
        $pdf = $this->getLayout()
            ->createBlock('simigiftvoucher/adminhtml_generategiftcard_edit_tab_giftcodelist')->getPdf();
     ...
     * Initialize action
     *
     * @return Simi_Simigiftvoucher_Adminhtml_GenerategiftcardController
     */
    ...
    public function editAction()
    {
        ...
        $model = Mage::getModel('simigiftvoucher/template')->load($id);
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('simigiftvoucher/adminhtml_generategiftcard_edit'))
                ->_addLeft($this->getLayout()->createBlock('simigiftvoucher/adminhtml_generategiftcard_edit_tabs'));

            $this->renderLayout();
        } else {
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('simigiftvoucher')->__('Template does not exist'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            $model = Mage::getModel('simigiftvoucher/template');
            ...
            if (!Mage::helper('simigiftvoucher')->isExpression($data['pattern'])) {
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('simigiftvoucher')->__('Invalid pattern'));
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('simigiftvoucher')->__('The pattern has been saved successfully.'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(
            Mage::helper('simigiftvoucher')->__('Unable to find Template to save'));
        $this->_redirect('*/*/');
    }
    ...
    public function generateAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            ...
            if ($model->getIsGenerated()) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('simigiftvoucher')->__('Each template only generate a time'));
                $this->_redirect('*/*/edit', array('id' => $model->getId()));
                return;
            }
            $data = $this->_filterDates($data, array('expired_at'));
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('simigiftvoucher')->__('The pattern has been saved and generated successfully.'));
                $this->_redirect('*/*/edit', array('id' => $model->getId()));
                return;
            } catch (Exception $e) {
    ...
    public function duplicateAction()
    {
        if ($this->getRequest()->getParam('id')) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('simigiftvoucher')->__('The pattern has been duplicated successfully.'));
                Mage::getSingleton('adminhtml/session')->setFormData(false);
        ...
        Mage::getSingleton('adminhtml/session')->addError(
            Mage::helper('simigiftvoucher')->__('Unable to find Template to duplicate'));
        $this->_redirect('*/*/');
    }
        ...
        if (!is_array($templateIds)) {
            ...
            try {
                foreach ($templateIds as $templateId) {
                    $template = Mage::getModel('simigiftvoucher/template')->load($templateId);
                    $template->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
    ...
    public function GiftcodelistAction()
    {
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('simigiftvoucher/adminhtml_generategiftcard_edit_tab_giftcodelist')->toHtml()
        );
    }
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('simigiftvoucher/generategiftcard');
    }
