...
/**
 * Class Simi_Simigiftvoucher_Block_Adminhtml_Gifttemplate_Edit_Tab_Images
 */
class Simi_Simigiftvoucher_Block_Adminhtml_Gifttemplate_Edit_Tab_Images extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _prepareForm() {
        ...
        if (isset($data['images']) && $data['images']) {
            ...
            if ($images) {
                ...
                foreach ($images as $image) {
                    ...
                    $str.='<img src=\"' . Mage::getBaseUrl("media") . 'simigiftvoucher/template/images/' . $type . $image . '\" alt=\"\" style=\"width:100px;height:100px\"
                    >';
                    $str.='<div style=\"position: absolute;top: 105px;\">';
                    $str.='<a class=\"preview-img\" href=\"javascript:previewImage(\'' . $image . '\')\">' . Mage::helper('simigiftvoucher')->__("Preview") . '</a>
                           ||<a class=\"remove-img\" href=\"javascript:removeImage(\'' . $image . '\')\">' . Mage::helper('simigiftvoucher')->__("Remove") . '</a>';
                    $str.='</div>';
                    $str.='</div>';
                }
        ...
        $fieldset = $form->addFieldset('images_fieldset', array('legend' => Mage::helper('simigiftvoucher')->__('Upload Images')));
...
//            'required' => true,
			...
			<span style="font-family:Arial"><span style="color: #003366">' . Mage::helper('simigiftvoucher')->__("Click to add file(s)") . '</span></span>
			<button id="addMore" title="Add more image" type="button" class="scalable add" onclick="AddFileUpload()"><span>' . Mage::helper("simigiftvoucher")->__("Add") . '</span></button>
			<span>&nbsp;&nbsp;' . Mage::helper('simigiftvoucher')->__('Recommended size: ') . '<span id="giftcard-notes-simple" style="display: none">584x310.</span><span id="giftcard-notes-top" style="display: none">600x190.</span><span id="giftcard-notes-center" style="display: none">600x365.</span><span id="giftcard-notes-left" style="display: none">250x365.</span>' . Mage::helper('simigiftvoucher')->__('&nbsp;&nbsp;Support gif, jpg, png files.') . '</span>
			<br /><br />
...
//            'required' => true,
            'after_element_html' => '<div id="fileuploaded" class="">
                <h3>' . Mage::helper("simigiftvoucher")->__("Uploaded images") . '</h3>
                <div></div>           
</div>' . '<script>
