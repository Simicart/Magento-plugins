...
class Simi_Simigiftvoucher_Helper_Data extends Mage_Core_Helper_Data
{
    ...
    public function getGeneralConfig($code, $store = null)
    {
        if ($code == 'barcode_enable' || $code == 'barcode_type') {
            return Mage::getStoreConfig('simigiftvoucher/print_voucher/' . $code, $store);
        }
        return Mage::getStoreConfig('simigiftvoucher/general/' . $code, $store);
    }
    ...
    {
        return Mage::getStoreConfig('simigiftvoucher/interface/' . $code, $store);
    }
    ...
    {
        return Mage::getStoreConfig('simigiftvoucher/interface_checkout/' . $code, $store);
    }
    ...
    {
        return Mage::getStoreConfig('simigiftvoucher/email/' . $code, $store);
    }
    ...
    {
        $option = explode(',', Mage::helper('simigiftvoucher')->getInterfaceCheckoutConfig('display'));
        $result = array();
    ...
    public function canUseCode($code)
    {
        ...
        if (is_string($code)) {
            $code = Mage::getModel('simigiftvoucher/giftvoucher')->loadByCode($code);
        }
        if (!($code instanceof Simi_Simigiftvoucher_Model_Giftvoucher)) {
            return false;
        }
        if (!$code->getId()) {
            return false;
    ...
    {
        return Mage::getUrl('simigiftvoucher/index/check');
    }
    ...
    public static function uploadImage($type)
    {
        ...
        if (strpos($type, 'image') !== false) {
            $imagePath = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'template' . DS . 'images' . DS;
            $imagePathEnd = 'images';
        } else {
            $imagePath = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'template' . DS . 'background' . DS;
        ...
        if (isset($_FILES[$type]['name']) && $_FILES[$type]['name'] != '') {
            try {
                ...
                self::resizeImage(Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . 'simigiftvoucher/template/' . 
                    $imagePathEnd . '/' . $result['file']);
                self::customResizeImage(Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . 
                    'simigiftvoucher/template/' . $imagePathEnd . '/', $result['file'], $imagePathEnd);
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')
                    ->addError(Mage::helper('simigiftvoucher')->__('The file uploaded has invalid format. Support jpg, jpeg, gif, png files only.'));
            }
        }
        return $image;
    }
    ...
    public static function createImageFolderHaitv($parent, $type, $tmp = false)
    {
        ...
        if ($tmp) {
            $imagePath = Mage::getBaseDir('media') . DS . 'tmp' . DS . 'simigiftvoucher' . DS . 'images' . DS;
        } else {
            $imagePath = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'template' . 
    ...
    public static function customResizeImage($imagePath, $imageName, $imageType)
    {
        ...
        if (file_exists($imageUrl)) {
            ...
            self::createImageFolderHaitv($imageType, 'amazon');
            if ($imageType == 'images') {
                $imageObj = new Varien_Image($imageUrl);
                $imageObj->constrainOnly(TRUE);
                $imageObj->keepAspectRatio(False);
                ...
                $imageObj->keepFrame(false);
                $imageObj->resize(584, 310);
                $imageObj->save($imagePath . 'amazon/' . $imageName);
            } else {
                $imageObj = new Varien_Image($imageUrl);
                $imageObj->constrainOnly(TRUE);
                $imageObj->keepAspectRatio(False);
    ...
    public function createBarcode($giftCode)
    {
        ...
        $imageUrl = Mage::getBaseDir('media') . DS . 'simigiftvoucher' . DS . 'template' . DS . 'barcode' . 
            DS . $giftCode . '.png';
    ...
    public function resize($width, $height = null)
    {
        $imageReturn = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . 
            "tmp/simigiftvoucher/cache/" . $this->_imageName;
        $this->_imageReturn = $imageReturn;
        if ($height == null) {
            $height = $width;
        }
        $imageUrl = Mage::getBaseDir('media') . DS . 'tmp' . DS . 'simigiftvoucher' . DS . 'cache' . 
     ...
     * @return bool
     */
    public function isShowOnlySimpleTemplate()
    {
        return (bool) Mage::getStoreConfig('simigiftvoucher/email/show_only_simple_template');
    }

    /**
     * @return mixed
     */
    public function getFoldPaperNote()
    {
        return Mage::getStoreConfig('simigiftvoucher/print_voucher/fold_paper_notes');
    }
