...
class Simi_Simigiftvoucher_Block_Payment_Form extends Mage_Payment_Block_Form
{

    protected $_checkoutLayout = 'simigiftvoucher/payment/form.phtml';
    ...
    protected function _construct()
    {
        ...
        if (Mage::helper('simigiftvoucher')->getGeneralConfig('active', $storeId) 
            && Mage::getStoreConfig('simigiftvoucher/interface_payment/show_gift_card')) {
            $class = get_class(Mage::app()->getFrontController()->getAction());
            if (isset($class) && $class != null) {
                $infor = explode('_', $class);
                $extensionName = $infor[0] . '_' . $infor[1];
                if (($extensionName != 'Mage_Checkout') && ($extensionName != 'Simi_Simigiftvoucher')) {
                    if (is_file(Mage::getDesign()->getTemplateFilename('simigiftvoucher' . DS . 'onestepcheckout' . DS . 
                        $extensionName . DS . 'form.phtml'))) {
                        $this->_checkoutLayout = 'simigiftvoucher/onestepcheckout/' . $extensionName . '/form.phtml';
                    }
                }
    ...
    {
        return Mage::getStoreConfig('payment/simigiftvoucher/description');
    }
     ...
     * @return array
     */
    public function getSimigiftVoucherDiscount()
    {
    ...
    {
        return trim($this->getUrl('simigiftvoucher/checkout/addgift'), '/');
    }
    ...
    {
        return Mage::helper('simigiftvoucher')->isModuleOutputEnabled('Simi_Onestepcheckout');
    }
