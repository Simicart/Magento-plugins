...
class Simi_Simigiftvoucher_Block_Product_View extends Mage_Catalog_Block_Product_View_Abstract
{
    ...
    protected function _prepareLayout()
    {
        ...
        if ($media && $_product->getTypeId() == 'simigiftvoucher') {
            $media->setTemplate('simigiftvoucher/product/media.phtml');
        }
    }
        ...
        if (!$this->hasData('gift_amount_description')) {
            ...
            if ($product->getShowGiftAmountDesc()) {
                if ($product->getGiftAmountDesc()) {
                    ...
                    $this->setData('gift_amount_description', 
                        Mage::helper('simigiftvoucher')->getInterfaceConfig('description')
                    );
                }
            }
        }
     ...
     * @return bool
     */
    public function getStyleDesignPattern($type){

        $templates = $this->getAvailableTemplate();
        $designPatternArr = array();
        foreach($templates as $template){
            $designPatternArr[] = $template['design_pattern'];
        }
        $key = array_search($type, $designPatternArr);
        if($key !== false){
    ...
    {
        $giftProduct = Mage::getModel('simigiftvoucher/product')->loadByProduct($this->getProduct());
