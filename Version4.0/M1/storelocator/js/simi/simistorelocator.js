...
var Simistorelocator = Class.create();
Simistorelocator.prototype = {
...
var SimistorelocatorFrontEnd = Class.create();
SimistorelocatorFrontEnd.prototype = {
            ...
            for (i=0; i<=this.markearryIdStore.length-1; i++){
                try{
                    if(this.markearryIdStore[i] == id){
                        $('simi-simistorelocator-getdirection-'+id).show();
                    }else{
                        $('simi-simistorelocator-getdirection-'+this.markearryIdStore[i]).hide();
                    }   
                }catch(err){
                    
                }                
            }
            this.directionsDisplay.setPanel(document.getElementById('simi-simistorelocator-directionsPanel-'+id));            
            this.calcRoute(id);
        }         
    },
    ...
    createRadius: function (radius){
        ...
        for(var i=this.markerArray.length;i--;){
            ...
            {              
                ...
                if(i<this.markerArray.length-1)
                    show = 1;
            }
        }
    ...
    calcRoute: function (idstore){
        destination_lat = $('simistorelocator-lat-'+idstore).value;
        destination_lng = $('simistorelocator-lng-'+idstore).value;
...
var SimistorelocatorObject = Class.create();
SimistorelocatorObject.prototype = {
