...
class Simi_Simistorelocator_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function saveImageStore($images, $id, $file, $radio) {
        foreach ($images as $item) {
            $mod = Mage::getModel('simistorelocator/image');
            ...
            if ($item['delete'] == 0) {
                ...
                $mod->setData('simistorelocator_id', $id);
    ...
    public function getConfig($nameConfig) {
        return Mage::getStoreConfig('simistorelocator/general/' . $nameConfig);
    }
    ...
    public function getBigImagebyStore($id_store) {
        $collection = Mage::getModel('simistorelocator/image')->getCollection()->addFieldToFilter('simistorelocator_id', $id_store)->addFieldToFilter('image_delete', 2);
        $url = "";
        foreach ($collection as $item) {
            if ($item->getData('name')) {
                if ($item->getData('statuses') == 1) {
                    $url = Mage::getBaseUrl('media') . 'simistorelocator/images/' . $id_store . '/' . $item->getData('options') . '/' . $item->getData('name');
                    break;
                } else {
                    $url = Mage::getBaseUrl('media') . 'simistorelocator/images/' . $id_store . '/' . $item->getData('options') . '/' . $item->getData('name');
                }
    ...
    public function deleteImageStore() {
        $image_info = Mage::getModel('simistorelocator/image')->getCollection()->addFilter('image_delete', 1);
        foreach ($image_info as $item) {
            $id = $item->getData('simistorelocator_id');
    ...
    public function getDataImage($id) {
        $collection = Mage::getModel('simistorelocator/image')->getCollection()->addFilter('simistorelocator_id', $id);
        return $collection;
    }

    public function getImageUrlJS() {
        $url = Mage::getBaseUrl('media') . 'simistorelocator/images/';
        return $url;
    }

    public function getImagePath($store_id, $options) {
        $path = Mage::getBaseDir('media') . DS . 'simistorelocator' . DS . 'images' . DS . $store_id . DS . $options;
        return $path;
    }

    public function getImagePathCache($id, $options) {
        $path = Mage::getBaseDir('media') . DS . 'simistorelocator' . DS . 'images' . DS . 'cache' . DS . $id . DS . $options;
        return $path;
    }
    ...
    public function deleteImageIcon($id, $image) {
        $image_path = Mage::getBaseDir('media') . DS . 'simistorelocator' . DS . 'images' . DS . 'icon' . DS . $id . DS . $image;
        try {
            if (file_exists($image_path)) {
                unlink($image_path);
            }
    ...
    public function getPathImageIcon($id) {
        $path = Mage::getBaseDir('media') . DS . 'simistorelocator' . DS . 'images' . DS . 'icon' . DS . $id;
        return $path;
    }
...
//        Zend_debug::dump($tags);die();
        foreach ($tags as $tag) {
            $modelTag = Mage::getModel('simistorelocator/tag');
            $modelTag->setData('value', $tag);
            $modelTag->setData('simistorelocator_id', $storeId);
            $modelTag->save();
        }
    }
    ...
    public function deleteTagFormStore($storeId) {
        if ($storeId) {
            $collectionTag = Mage::getModel('simistorelocator/tag')->getCollection()
                    ->addFieldToFilter('simistorelocator_id', $storeId);
    ...
    public function getImageNameByStore($storeId) {
        if ($storeId) {
            ...
            $collectionImages = Mage::getModel('simistorelocator/image')->getCollection()
                    ->addFieldToFilter('simistorelocator_id', $storeId);
            $image_names = "";
    ...
    public function deleteImageFormStore($storeId) {
        if ($storeId) {
            $collectionImage = Mage::getModel('simistorelocator/image')->getCollection()
                    ->addFieldToFilter('simistorelocator_id', $storeId);
    ...
    public function getSpecialDays($storeId) {
        $specialdays = Mage::getModel('simistorelocator/specialday')
                ->getCollection()
                ->addFieldToFilter('store_id', array('finset' => $storeId));

        $day_show = Mage::getStoreConfig('simistorelocator/general/show_spencial_days', Mage::app()->getStore()->getStoreId());
    ...
    public function getHolidayDays($storeId) {
        $holidays = Mage::getModel('simistorelocator/holiday')
                ->getCollection()
                ->addFieldToFilter('store_id', array('finset' => $storeId));
