...
class Simi_Simistorelocator_Model_Api extends Simi_Connector_Model_Abstract {
    ...
    public function getStoreListCollection() {
        $collections = Mage::getModel('simistorelocator/simistorelocator')->getCollection()
    ...
    public function convertData($collection) {
        ...
        foreach ($collection as $item) {
            ...
            $storeInfo["special_days"] = Mage::helper('simistorelocator')->getSpecialDays($item->getId());
            $storeInfo["holiday_days"] = Mage::helper('simistorelocator')->getHolidayDays($item->getId());
            $data[] = $storeInfo;
        }
        $info["stores"] = $data;
    ...
    public function getSearchConfig() {
        $choose_search = Mage::helper('simistorelocator')->getConfig('choose_search');
    ...
    public function getStoreByDistance($data) {
        ...
        foreach ($collection as $item) {
            if (count($storeIds) != 0) {
                if (in_array($item->getId(), $storeIds)) {
                    ...
                    $data["image"] = Mage::helper("simistorelocator")->getBigImagebyStore($item->getId());
        ...
        foreach ($this->_arrStore as $store) {
            ...
            $store["special_days"] = Mage::helper('simistorelocator')->getSpecialDays($store["simistorelocator_id"]);
            $store["holiday_days"] = Mage::helper('simistorelocator')->getHolidayDays($store["simistorelocator_id"]);
            $storeList[] = $store;
        }
    ...
    public function getAllowedCountries() {
        $list = array();
        $country_default = Mage::helper('simistorelocator')->getConfig('default_country');
