...
class Simi_Simistorelocator_Model_Simistorelocator extends Mage_Core_Model_Abstract {
    ...
    protected function _beforeSave() {
        if ($storeId = $this->getStoreId()) {
            $defaultStore = Mage::getModel('simistorelocator/simistorelocator')->load($this->getId());
    ...
    protected function _afterSave() {
        if ($storeId = $this->getStoreId()) {
            ...
            foreach ($storeAttributes as $attribute) {
                $attributeValue = Mage::getModel('simistorelocator/storevalue')
...
//        Zend_debug::dump($this->getData());die();
    ...
    public function getMultiStoreValue($storeId = null) {
        ...
        $storeValues = Mage::getModel('simistorelocator/storevalue')->getCollection()
                ->addFieldToFilter('simistorelocator_id', $this->getId())
    ...
    public function save() {
        ...
        if ($addressFull) {
            ...
            if (strcasecmp($addressFull, $currentAddress)) {
                $coordinates = Mage::getModel('simistorelocator/gmap')
        ...
        if ($option == 1) {
            ...
            if (count($collection)) {
                foreach ($collection as $item) {
                    $this->setData($data);
                    $this->setId($item->getData('simistorelocator_id'));
                    $this->save();
                }
            }else{ 
                $this->setData($data);
