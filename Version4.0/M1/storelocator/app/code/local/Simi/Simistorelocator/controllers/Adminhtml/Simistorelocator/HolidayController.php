...
class Simi_Simistorelocator_Adminhtml_Simistorelocator_HolidayController extends Mage_Adminhtml_Controller_Action {

    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('simistorelocator/holidays')
    ...
    public function editAction() {
        ...
        $model = Mage::getModel('simistorelocator/holiday')->load($id);
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->loadLayout();
            $this->_setActiveMenu('simistorelocator/holidays');
            ...
            $this->_addContent($this->getLayout()->createBlock('simistorelocator/adminhtml_holiday_edit'))
                    ->_addLeft($this->getLayout()->createBlock('simistorelocator/adminhtml_holiday_edit_tabs'));
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            $model = Mage::getModel('simistorelocator/holiday');
            if($data['store_id']){    
                $data['store_id'] = implode(',',$data['store_id']);
               }
            if ($this->getRequest()->getParam('id')) {
                $holidays = $model->getCollection()
                        ->addFieldToFilter('simistorelocator_holiday_id', array('nin' => $this->getRequest()->getParam('id')))
            ...
            if (!($holidays->getData())) {              
                ...
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('simistorelocator')->__('Holiday exists'));
                Mage::getSingleton('adminhtml/session')->setFormData($data);
                $this->_redirect('*/*/edit', array('id' => $this->getRequest()->getParam('id')));
                return;
            }
    ...
    public function massDeleteAction() {
        $simistorelocatorIds = $this->getRequest()->getParam('holiday');
        if (!is_array($simistorelocatorIds)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($simistorelocatorIds as $simistorelocatorId) {
                    $simistorelocator = Mage::getModel('simistorelocator/holiday')->load($simistorelocatorId);
    ...
    public function exportCsvAction() {
        ...
        $content = $this->getLayout()->createBlock('simistorelocator/adminhtml_holiday_grid')
                ->getCsv();
