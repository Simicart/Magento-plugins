...
class Simi_Simistorelocator_Block_Adminhtml_Specialday_Grid extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    protected function _prepareCollection() {
        ...
        if(Mage::helper('simiconnector/cloud')->getWebsiteIdSimiUser()) {
            ...
            foreach ($collection as $special){
                $specialStoreId = explode(',', $special->getStoreId());
                $intersectArray = array_intersect($storelocatorIds,$specialStoreId);
                if(sizeof($intersectArray) > 0){
                    $cacheCollection->addItem($special);
                }
            }
