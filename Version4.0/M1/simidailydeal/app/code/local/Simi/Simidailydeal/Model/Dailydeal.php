...
class Simi_Simidailydeal_Model_Dailydeal extends Mage_Core_Model_Abstract
{
	...
	{
		parent::_construct();
		$this->_init('simidailydeal/dailydeal');
	}
	...
	public function getDailydeals()
	{
        ...
        foreach($deals as $entry){
            if ($entry->getQuantity() == $entry->getSold()){
                $entry['status'] = 2;
            }
            $entry->save();
        }
        ...
        $expired_deals  = $this->getCollection()
                               ->addFieldToFilter('close_time',array('lt'=>$current_time));

        //zend_debug::dump($current_time);
		...
		if (is_null($this->_dealsCollection)) {
            ...
            //zend_debug::dump($deals->getData());die;
	...
	public function getSidebar($limit = 3, $method = '')
	{
		...
		switch ($method) {
			case Simi_Simidailydeal_Model_System_Config_Display::RANDOM:
				...
				break;
			case Simi_Simidailydeal_Model_System_Config_Display::TODAY:
			...
			case Simi_Simidailydeal_Model_System_Config_Display::THIS_WEEK:
			...
			case Simi_Simidailydeal_Model_System_Config_Display::THIS_MONTH:
	...
	public function getDealByProduct($productId)
	{
		...
		//zend_debug::dump($deal->getData());
        $current_time = date("Y-m-d H:i:s", Mage::getModel('core/date')->timestamp(time()));
        $deal->getSelect()->where('(start_time <= ?)', $current_time);
		if ($deal->getSize())
			return $deal->getFirstItem();

		return Mage::getModel('simidailydeal/dailydeal');
	}
