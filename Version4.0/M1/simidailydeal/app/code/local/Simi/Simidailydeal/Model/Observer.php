...
class Simi_Simidailydeal_Model_Observer
{
	public function collectionLoadAfter($observer)
	{
		...
		foreach ($productCollection as $product) {
			//getDealByProduct($product->getEntityId());
	...
	public function checkOutSaveAfter($observer)
	{
		...
		$i     = 0;
		foreach ($items as $item) {
			$deal = Mage::getModel('simidailydeal/dailydeal')->getDealByProduct($item->getProductId());
			if ($deal->getId()) {
	...
	public function refundCreditmemo($observer)
	{
		...
		foreach ($deals_arr as $value) {
			$deal  = Mage::getModel('simidailydeal/dailydeal')->load($value);
