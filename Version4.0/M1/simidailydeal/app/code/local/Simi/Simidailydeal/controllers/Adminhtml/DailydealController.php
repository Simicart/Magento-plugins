...
class Simi_Simidailydeal_Adminhtml_DailydealController extends Mage_Adminhtml_Controller_Action
{
	protected function _initAction()
	{
		$this->loadLayout()
			->_setActiveMenu('simidailydeal/dailydeal')
	...
	public function editAction()
	{
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('simidailydeal/adminhtml_dailydeal_edit'))
				->_addLeft($this->getLayout()->createBlock('simidailydeal/adminhtml_dailydeal_edit_tabs'));
	...
	public function saveAction()
	{
		if ($data = $this->getRequest()->getPost()) {
			...
			$model = Mage::getModel('simidailydeal/dailydeal')->load($this->getRequest()->getParam('id'));
	...
	public function deleteAction()
	{
		if ($this->getRequest()->getParam('id') > 0) {
			try {
				$model = Mage::getModel('simidailydeal/dailydeal');
				$model->delete();
	...
	{
		$dealIds = $this->getRequest()->getParam('simidailydeal');
		if (!is_array($dealIds)) {
			...
			try {
				...
				foreach ($dealIds as $dealId) {
					$deal = Mage::getModel('simidailydeal/dailydeal')->load($dealId);
					$deal->delete();
					$i++;
				}
				if ($i > 0) {
	...
	public function massStatusAction()
	{
		...
		if (!is_array($dealIds)) {
			...
			try {
				foreach ($dealIds as $dealId) {
					$deal = Mage::getSingleton('simidailydeal/dailydeal')
						->load($dealId)
	...
	public function exportCsvAction()
	{
		...
		$content  = $this->getLayout()->createBlock('simidailydeal/adminhtml_dailydeal_grid')->getCsv();
	...
	public function exportXmlAction()
	{
		...
		$content  = $this->getLayout()->createBlock('simidailydeal/adminhtml_dailydeal_grid')->getXml();
		$this->_prepareDownloadResponse($fileName, $content);
	}
	...
	{
		$this->loadLayout();
		$this->getLayout()->getBlockSingleton('simidailydeal/adminhtml_dailydeal_edit_tab_listproduct')
