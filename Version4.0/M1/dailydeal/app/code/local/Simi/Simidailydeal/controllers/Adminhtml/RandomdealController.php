...
class Simi_Simidailydeal_Adminhtml_RandomdealController extends Mage_Adminhtml_Controller_Action
{
	protected function _initAction(){
			if(!Mage::registry('is_random_simidailydeal'))
                Mage::helper('simidailydeal')->updateSimidailydealStatus();
		$this->loadLayout()
			->_setActiveMenu('simidailydeal/randomdeal')
	...
	public function editAction() {
		if(!Mage::helper('magenotification')->checkLicenseKeyAdminController($this)){ return; }
        Mage::getSingleton('adminhtml/session')->addNotice(Mage::helper('simidailydeal')->__('Deal Generator helps you create deals automatically in random mode. Please select products for which you would like to create deal, then fill in the information pattern below.'));
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('simidailydeal/adminhtml_randomdeal_edit'))
				->_addLeft($this->getLayout()->createBlock('simidailydeal/adminhtml_randomdeal_edit_tabs'));

			$this->renderLayout();
		} else {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('simidailydeal')->__('Deal Generator does not exist'));
			$this->_redirect('*/*/');
		}
	}
	...
	public function newAction() {
		...
		if ($data = $this->getRequest()->getPost()) {
			...
			try {
				...
				if(!Mage::registry('is_random_simidailydeal'))
                                Mage::helper('simidailydeal')->updateSimidailydealStatus();
				Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('simidailydeal')->__('Deal generator was successfully saved'));
		...
		Mage::getSingleton('adminhtml/session')->addError(Mage::helper('simidailydeal')->__('Unable to find deal generator to save'));
		$this->_redirect('*/*/');
	}
	...
	public function massDeleteAction() {
		...
		if(!is_array($randomdealIds)){
			...
			try {
				...
				foreach ($randomdealIds as $randomdealId) {
					$randomdeal= Mage::getModel('simidailydeal/simidailydeal')->load($randomdealId);
		...
		if(!is_array($randomdealIds)) {
			...
			try {
				foreach ($randomdealIds as $randomdealId) {
					$randomdeal = Mage::getSingleton('simidailydeal/simidailydeal')
						->load($randomdealId)
                                                ->setProductId(0)
		...
		$content	= $this->getLayout()->createBlock('simidailydeal/adminhtml_randomdeal_grid')->getCsv();
		$this->_prepareDownloadResponse($fileName,$content);
	}

	public function exportXmlAction(){
		$fileName   = 'randomdeal.xml';
		$content	= $this->getLayout()->createBlock('simidailydeal/adminhtml_randomdeal_grid')->getXml();
		$this->_prepareDownloadResponse($fileName,$content);
	}
