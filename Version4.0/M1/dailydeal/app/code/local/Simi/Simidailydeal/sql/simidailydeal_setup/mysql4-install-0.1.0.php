...
ALTER TABLE {$this->getTable('sales/order')}
  DROP COLUMN `simidailydeals`;    
");
}catch(Exception $e){}
$installer->run("

DROP TABLE IF EXISTS {$this->getTable('simidailydeal')};
CREATE TABLE {$this->getTable('simidailydeal')} (
...
ALTER TABLE {$this->getTable('sales/order')}
  ADD COLUMN `simidailydeals` varchar(255) NULL;
    ");
