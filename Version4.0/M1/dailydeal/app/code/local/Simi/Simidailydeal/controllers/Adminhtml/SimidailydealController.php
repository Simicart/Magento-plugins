...
class Simi_Simidailydeal_Adminhtml_SimidailydealController extends Mage_Adminhtml_Controller_Action
{
	protected function _initAction(){
		if(!Mage::registry('is_random_simidailydeal'))
        Mage::helper('simidailydeal')->updateSimidailydealStatus();
		$this->loadLayout()
			->_setActiveMenu('simidailydeal/simidailydeal')
	...
	public function editAction() {
		...
		$model  = Mage::getModel('simidailydeal/simidailydeal')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->loadLayout();
			$this->_setActiveMenu('simidailydeal/simidailydeal');
			...
			$this->_addContent($this->getLayout()->createBlock('simidailydeal/adminhtml_simidailydeal_edit'))
				->_addLeft($this->getLayout()->createBlock('simidailydeal/adminhtml_simidailydeal_edit_tabs'));

			$this->renderLayout();
		} else {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('simidailydeal')->__('Deal does not exist'));
			$this->_redirect('*/*/');
		}
	}
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
				...
				try {
					...
					$uploader1->save($path_thumbnail_image.'simidailydeal/main', $_FILES['thumbnail']['name'] );
					
				} catch (Exception $e) {}
				
				//this way the name is saved in DB
	  			$data['thumbnail_image'] ='simidailydeal/main/'.$_FILES['thumbnail']['name'];
			}
			...
			$model = Mage::getModel('simidailydeal/simidailydeal')->load($this->getRequest()->getParam('id'));
			...
			try {
                            if ($price>=$data['deal_price']){
                                if (Mage::getModel('core/date')->timestamp($data['start_time'])<=Mage::getModel('core/date')->timestamp($data['close_time'])){
										...
										if(!Mage::registry('is_random_simidailydeal'))
                                        Mage::helper('simidailydeal')->updateSimidailydealStatus();
                                        Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('simidailydeal')->__('Deal was successfully saved'));
                                }else {
                                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('simidailydeal')->__('Start time must be smaller than close time!'));
                                }
                            }else{
                               Mage::getSingleton('adminhtml/session')->addError(Mage::helper('simidailydeal')->__('Deal Price must be smaller than Product price')); 
                            }
		...
		Mage::getSingleton('adminhtml/session')->addError(Mage::helper('simidailydeal')->__('Unable to find deal to save'));
		$this->_redirect('*/*/');
	}
	...
	public function deleteAction() {
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				$model = Mage::getModel('simidailydeal/simidailydeal');
	...
	public function massDeleteAction() {
		$simidailydealIds = $this->getRequest()->getParam('simidailydeal');
		if(!is_array($simidailydealIds)){
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select Deal(s)'));
		}else{
			try {
                                $i=0;
				foreach ($simidailydealIds as $simidailydealId) {
					$simidailydeal= Mage::getModel('simidailydeal/simidailydeal')->load($simidailydealId);
                                        $t=$simidailydeal->getStatus();
                                        if (($t==4)||($t==2))
                                        {
                                            $simidailydeal->delete();
                                            $i++;
                                        }
				}
                                if ($i>0){
	...
	public function massStatusAction() {
		...
		if(!is_array($simidailydealIds)) {
			...
			try {
				foreach ($simidailydealIds as $simidailydealId) {
					$simidailydeal = Mage::getSingleton('simidailydeal/simidailydeal')
						->load($simidailydealId)
						->setStatus($this->getRequest()->getParam('status'))
						->setIsMassupdate(true)
						->save();
				}
				$this->_getSession()->addSuccess(
					$this->__('Total of %d Deal(s) were successfully updated', count($simidailydealIds))
				);
			} catch (Exception $e) {
	...
	public function exportCsvAction(){
		$fileName   = 'simidailydeal.csv';
		$content	= $this->getLayout()->createBlock('simidailydeal/adminhtml_simidailydeal_grid')->getCsv();
		$this->_prepareDownloadResponse($fileName,$content);
	}

	public function exportXmlAction(){
		$fileName   = 'simidailydeal.xml';
		$content	= $this->getLayout()->createBlock('simidailydeal/adminhtml_simidailydeal_grid')->getXml();
		$this->_prepareDownloadResponse($fileName,$content);
	}
	...
	{
        $this->loadLayout();
        $this->getLayout()->getBlock('simidailydeal.edit.tab.listproduct')
	...
	{
        $this->loadLayout();
        $this->getLayout()->getBlock('simidailydeal.edit.tab.listorder')
