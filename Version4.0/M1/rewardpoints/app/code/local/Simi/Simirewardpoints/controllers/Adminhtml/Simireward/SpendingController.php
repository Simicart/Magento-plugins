...
/**
 * Simirewardpoints Spending Adminhtml Controller
 * 
 * @category    Simi
 * @package     Simi_Simirewardpoints
 * @author      Simi Developer
 */
class Simi_Simirewardpoints_Adminhtml_Simireward_SpendingController extends Mage_Adminhtml_Controller_Action
{
    /**
     * init layout and set active for current menu
     *
     * @return Simi_Simirewardpoints_Adminhtml_SpendingController
     */
    protected function _initAction()
    {
        $this->loadLayout()
            ->_setActiveMenu('simirewardpoints/spending')
            ->_addBreadcrumb(
    ...
    public function editAction()
    {
        ...
        if ($model->getId() || $rateId == 0) {
            ...
            $this->_setActiveMenu('simirewardpoints/spending');
            ...
            $this->_addContent($this->getLayout()->createBlock('simirewardpoints/adminhtml_spending_edit'))
                ->_addLeft($this->getLayout()->createBlock('simirewardpoints/adminhtml_spending_edit_tabs'));
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                if (!$model->getDirection()) {
                    $model->setDirection(Simi_Simirewardpoints_Model_Rate::POINT_TO_MONEY);
                }
                $model->save();
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('simirewardpoints')->__('Spending rate was successfully saved')
                );
