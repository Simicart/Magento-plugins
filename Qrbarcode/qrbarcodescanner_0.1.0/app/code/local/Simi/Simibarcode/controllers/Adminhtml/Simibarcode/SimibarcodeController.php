 ...
 */
class Simi_Simibarcode_Adminhtml_Simibarcode_SimibarcodeController extends Mage_Adminhtml_Controller_Action
{
	/**
    ...
    {
		$this->loadLayout()
			->_setActiveMenu('simibarcode/simibarcode')
	...
	public function editAction() 
    {
        ...
        $model = Mage::getModel('simibarcode/simibarcode')->load($simibarcodeId);
        ...
        if ($model->getId() || $simibarcodeId == 0) {
            ...
            $this->loadLayout();
            $this->_setActiveMenu('simibarcode/simibarcode');

            $this->_addBreadcrumb(
                    Mage::helper('adminhtml')->__('Manage Barcodes'), 
                    Mage::helper('adminhtml')->__('Manage Barcodes')
            );
            $this->_addBreadcrumb(
                    Mage::helper('adminhtml')->__('Add New Custom Barcode'), 
                    Mage::helper('adminhtml')->__('Add New Custom Barcode')
            );
            if (!$simibarcodeId) {
            ...
            $this->_addContent($this->getLayout()->createBlock('simibarcode/adminhtml_simibarcode_edit'))
				->_addLeft($this->getLayout()->createBlock('simibarcode/adminhtml_simibarcode_edit_tabs'));

			$this->renderLayout();
		} else {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('simibarcode')
    ...
    {
        $code = Mage::helper('simibarcode')->generateCode(Mage::getStoreConfig('simibarcode/barcode/pattern'));
    ...
    {
        $code = Mage::helper('simibarcode')->generateCode(Mage::getStoreConfig('simibarcode/barcode/qrcode_pattern'));
	...
	public function saveAction() 
    {
		if ($post = $this->getRequest()->getPost()) {
            $model = Mage::getModel('simibarcode/simibarcode')->load($this->getRequest()->getParam('id'));
            try {
                if ($model->getId()) {
                    ...
                    if ($this->getRequest()->getParam('back')) {
                        Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('simibarcode')->__('Barcode "%s" was successfully edited.', $model->getBarcode()));
                ...
                $tablename = 'simibarcode/simibarcode';

                $results = Mage::helper('simibarcode')->getAllColumOfTable($tablename);
                    ...
                    if (count($products)) {
                        ...
                        foreach ($products as $pId => $enCoded) {
                            ...
                            if ($codeArr['barcode'] == '') {
                                ...
                                if (!$model->getId()) {
                                    $checkBarcodeExist = Mage::getModel('simibarcode/simibarcode')->load($codeArr['barcode'], 'barcode');
                                    if ($checkBarcodeExist->getId()) {
                                        Mage::getSingleton('adminhtml/session')->addError(
                                                Mage::helper('simibarcode')->__('The barcode "%s" was already exist!', $codeArr['barcode'])
                                        );
                                ...
                                if (in_array($codeArr['barcode'], $barcode)) {
                                    Mage::getSingleton('adminhtml/session')->addError(
                                            Mage::helper('simibarcode')->__('The barcode "%s" was already duplicate!', $codeArr['barcode'])
                                    );
                                    Mage::getSingleton('adminhtml/session')->setFormData($post);
                                    $this->_redirect('*/*/edit', array('id' => $model->getId()));
                                    return;
                            ...
                            if ($codeArr['qrcode'] == '') {
                                ...
                                //check barcode already exist
                                if (!$model->getId()) {
                                    $checkQrcodeExist = Mage::getModel('simibarcode/simibarcode')->load($codeArr['qrcode'], 'qrcode');
                                    if ($checkQrcodeExist->getId()) {
                                        Mage::getSingleton('adminhtml/session')->addError(
                                                Mage::helper('simibarcode')->__('The QRcode "%s" was already exist!', $codeArr['qrcode'])
                                        );
                                        Mage::getSingleton('adminhtml/session')->setFormData($post);
                                        $this->_redirect('*/*/edit', array('id' => $model->getId()));
                                        return;
                                ...
                                if (in_array($codeArr['qrcode'], $qrcode)) {
                                    Mage::getSingleton('adminhtml/session')->addError(
                                            Mage::helper('simibarcode')->__('The QRcode "%s" was already duplicate!', $codeArr['qrcode'])
                                    );
                                    Mage::getSingleton('adminhtml/session')->setFormData($post);
                                    $this->_redirect('*/*/edit', array('id' => $model->getId()));
                                    return;
                            ...
                            foreach ($columns as $id => $column) {
                                ...
                                if ($_id != 'custom') {
                                    ...
                                    $return = Mage::helper('simibarcode')->getValueForBarcode($_id, $key, $pId, $codeArr);
                                    if (is_array($return)) {
                            ...
                            if (count($sqlNews) == 1000) {
                                $writeConnection->insertMultiple($resource->getTableName('simibarcode/simibarcode'), $sqlNews);
                                $sqlNews = array();
                            }
                ...
                if ($this->getRequest()->getParam('back')) {
                    Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('simibarcode')->__('Barcode was successfully saved.'));
                    $this->_redirect('*/*/new');
                    return;
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('simibarcode')->__('Barcode was successfully saved.'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('simibarcode')->__('Unable to find barcode to save!')
        );
        $this->_redirect('*/*/');
	}
	...
	public function deleteAction() 
    {
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				$model = Mage::getModel('simibarcode/simibarcode');
		...
		if(!is_array($simibarcodeIds)){
			...
			try {
				foreach ($simibarcodeIds as $simibarcodeId) {
					$simibarcode = Mage::getModel('simibarcode/simibarcode')->load($simibarcodeId);
					$simibarcode->delete();
				}
	...
	public function massStatusAction() 
    {
		...
		if(!is_array($simibarcodeIds)) {
			...
			try {
				foreach ($simibarcodeIds as $simibarcodeId) {
					$simibarcode = Mage::getSingleton('simibarcode/simibarcode')
						->load($simibarcodeId)
    ...
    {
		$fileName   = 'simibarcode.csv';
		$content	= $this->getLayout()->createBlock('simibarcode/adminhtml_simibarcode_grid')->getCsv();
    ...
    {
		$fileName   = 'simibarcode.xml';
		$content	= $this->getLayout()->createBlock('simibarcode/adminhtml_simibarcode_grid')->getXml();
		$this->_prepareDownloadResponse($fileName,$content);
	}
	...
	public function getImportCsvAction() 
    {
        if (isset($_FILES['fileToUpload']['name']) && $_FILES['fileToUpload']['name'] != '') {
            ...
            try {
                ...
                $fields = array();
                $count = 0;
                $helper = Mage::helper('simibarcode');
