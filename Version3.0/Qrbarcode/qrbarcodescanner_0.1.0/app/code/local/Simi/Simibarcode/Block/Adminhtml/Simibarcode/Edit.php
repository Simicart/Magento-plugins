 ...
 */
class Simi_Simibarcode_Block_Adminhtml_Simibarcode_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
	public function __construct()
    {
		...
		$this->_updateButton('save', 'label', Mage::helper('simibarcode')->__('Save Item'));
        ...
        if($this->getRequest()->getParam('id')){
            ...
            $this->_addButton('print_barcode', array(
                'label' => Mage::helper('simibarcode')->__('Print Barcodes'),
                'onclick' => 'createBarcode()',
                'class' => 'add',
                    ), 0);   

            $this->_addButton('print_qrcode', array(
                'label' => Mage::helper('simibarcode')->__('Print QR codes'),
                'onclick' => 'createQrcode()',
                'class' => 'add',
                    ), 0);                   
        }
        $classBarcode = Mage::helper('simibarcode')->getValidateBarcode();
		...
		$this->_formScripts[] = "
           ...
           function uploadFailed(evt) {
                alert('".Mage::helper('simibarcode')->__('There was an error attempting to upload the file.')."');
           }

           function uploadCanceled(evt) {
                alert('".Mage::helper('simibarcode')->__('The upload has been canceled by the user or the browser dropped the connection.')."');
           }   
        ";
	}
	...
	public function getHeaderText()
    {
		if (Mage::registry('simibarcode_data') && Mage::registry('simibarcode_data')->getId()
            ...
            return Mage::helper('simibarcode')->__("Edit Code '%s'", $this->htmlEscape(Mage::registry('simibarcode_data')->getBarcode())
            );
        }
        return Mage::helper('simibarcode')->__('Add QR & Barcode ');
	}
}