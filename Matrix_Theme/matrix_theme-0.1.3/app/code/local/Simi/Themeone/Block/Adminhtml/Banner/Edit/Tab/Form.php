 ...
 */
class Simi_Themeone_Block_Adminhtml_Banner_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _prepareForm() {
        ...
        $fieldset = $form->addFieldset('themeone_form', array('legend' => Mage::helper('themeone')->__('Banner information')));

        $fieldset->addField('website_id', 'select', array(
            'label' => Mage::helper('themeone')->__('Website'),
            'name' => 'website_id',
            'values' => Mage::getSingleton('themeone/status')->getWebsite(),
            'disabled'=>true
        ));
        ...
        $fieldset->addField('type', 'select', array(
            'label' => Mage::helper('themeone')->__('Direct viewers to'),
            'class' => 'required-entry',
            'required' => true,
            'name' => 'type',
            'values' => Mage::getModel('themeone/banner')->toOptionArray(),
            'onchange' => 'onchangeNoticeType(this.value)',
            'after_element_html' => '<script> Event.observe(window, "load", function(){onchangeNoticeType(\''.$type.'\');});</script>',
        ));
        ...
        $fieldset->addField('product_id', 'text', array(
            ...
            'class' => 'required-entry',
            'required' => true,
            'label' => Mage::helper('themeone')->__('Product ID'),
            'note'  => Mage::helper('themeone')->__('Choose a product'),
                ...
                <script type="text/javascript">
                    function toggleMainProducts(){
                        if($("main_products_select").style.display == "none"){
                            var url = "' . $this->getUrl('adminhtml/themeone_banner/chooserMainProducts') . '";
        ...
        $fieldset->addField('category_id', 'text', array(
            ...
            'class' => 'required-entry',
            'required' => true,
            'label' => Mage::helper('themeone')->__('Category ID'),
            'note'  => Mage::helper('themeone')->__('Choose a category'),
                ...
                <div id="main_categories_select" style="display:none"></div>
                    ...
                    function toggleMainCategories(check){
                        ...
                        if($("main_categories_select").style.display == "none" || (check ==1) || (check == 2)){
                            var url = "' . $this->getUrl('adminhtml/themeone_banner/chooserMainCategories') . '";                        
                            if(check == 1){
        ...
        $fieldset->addField('banner_url', 'editor', array(
            'name' => 'banner_url',           
            'label' => Mage::helper('themeone')->__('URL'),
            'title' => Mage::helper('themeone')->__('URL'),
			'note' => Mage::helper('themeone')->__('Please enter the URL to which customers will be directed when they tap on'),
        ));
